...
// Matrix: modulo -> [TERMINAL, CAJA, SUPERVISOR, BODEGA, SOPORTE]
const MATRIX = {
  dashboard: [V, N, V, V],
  pos: [VC, N, N, N],
  entradas: [N, N, VCE, VC],
...
const ROLES = ["TERMINAL", "CAJA", "SUPERVISOR", "BODEGA", "SOPORTE"];
...
try {
  ...
  for (const modulo of MODULOS) {
    ...
    for (let i = 0; i < ROLES.length; i++) {
      const rol = ROLES[i];
      // SOPORTE starts denied outside the explicitly assigned support
  ...
  console.log(
    "Seed completado: ubicaciones, admin, ticket_folio y matriz de permisos para TERMINAL, CAJA, SUPERVISOR, BODEGA y SOPORTE.",
  );
  console.log(
