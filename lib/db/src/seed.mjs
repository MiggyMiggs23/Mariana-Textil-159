...
// Matrix: modulo -> [TERMINAL, CAJA, SUPERVISOR, BODEGA, SISTEMAS]
const MATRIX = {
  dashboard: [V, N, V, V],
  pos: [VC, N, N, N],
  entradas: [N, N, VCE, VC],
  ...
  auditoria: [N, N, N, N, N],
  camionetas: [N, N, N, N, VCE],
  choferes: [N, N, N, N, VCE],
  viajes: [N, N, VC, VC, VCE],
};

const ROLES = ["TERMINAL", "CAJA", "SUPERVISOR", "BODEGA", "SISTEMAS"];
...
try {
  ...
  try {
    ...
    await migrationClient.query("SELECT pg_advisory_xact_lock(310031)");
  ...
  for (const modulo of MODULOS) {
    ...
    for (let i = 0; i < ROLES.length; i++) {
      const rol = ROLES[i];
      // SISTEMAS starts denied outside the explicitly assigned support
      // catalogs. Existing explicit customizations remain untouched.
  ...
  console.log(
    "Seed completado: ubicaciones, admin, ticket_folio y matriz de permisos para TERMINAL, CAJA, SUPERVISOR, BODEGA y SISTEMAS.",
  );
  console.log(
