...
test("la matriz cubre los 32 módulos con seis roles", () => {
...
test("SISTEMAS y CONTADOR conservan los permisos financieros definidos", () => {
  const sistemas = [
    ...
    VE, N, N, N, V, VC, V, VCE, VCE, VCE, N, VCE,
  ];
  const contador = [
    V, N, V, V, V, N, V, N, V, V,
    N, V, V, V, V, V, VCE, V, N, N,
    N, V, V, VC, V, V, N, N, N, V, N, N,
  ];
...
test("BODEGA no hereda Contenedores y coincide con la revocación de costos pendientes", async () => {
  const bodegaIndex = ROLES.indexOf("BODEGA");
  assert.deepEqual(MATRIX.contenedores[bodegaIndex], N);

  const upgrade = await readFile(
    new URL("./pending-costs-schema.ts", import.meta.url),
    "utf8",
  );
  const revokedDefaults = upgrade.match(/modulo NOT IN\s*\(([^)]+)\)/);
  assert.ok(revokedDefaults, "no se encontró la lista de defaults permitidos");
  assert.doesNotMatch(revokedDefaults[1], /['"]contenedores['"]/);
});
