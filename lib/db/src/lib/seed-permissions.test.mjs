...
test("la matriz cubre los 29 módulos con seis roles", () => {
  assert.equal(MODULOS.length, 29);
...
test("SISTEMAS y CONTADOR conservan los permisos financieros definidos", () => {
  const sistemas = [
    V, N, VCE, VCE, VCE, VC, V, VCE, VCE, VC,
    VCE, V, V, V, VCE, V, VCE, VCE, VCE, VE,
    N, N, N, V, VC, V, VCE, VCE, VCE,
  ];
  const contador = [
    V, N, V, V, V, N, V, V, V, N,
    V, V, V, V, V, VCE, V, N, N, N,
    V, V, VC, V, V, N, N, N, V,
  ];
