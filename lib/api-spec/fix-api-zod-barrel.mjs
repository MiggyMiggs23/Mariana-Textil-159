/**
 * Post-codegen script: rewrites lib/api-zod/src/index.ts to avoid
 * TypeScript TS2308 collision between Orval's Zod path-param schemas
 * (in generated/api.ts) and the query-param TS interfaces (in generated/types/).
 *
 * Orval always appends `export * from './generated/types'` to src/index.ts,
 * causing 4 name collisions:
 *   - ListComprasProveedorParams
 *   - EstadoCuentaProveedorParams
 *   - EstadisticasProveedorParams
 *   - ExportarProveedorXlsxParams
 *
 * This script replaces the barrel with a selective re-export that skips those
 * four and re-exports them under a `QueryParams` suffix alias instead.
 */

import { readFileSync, writeFileSync, readdirSync } from "fs";
import { resolve, dirname } from "path";
import { fileURLToPath } from "url";

const __dirname = dirname(fileURLToPath(import.meta.url));
const root = resolve(__dirname, "..", "..");
const apiZodSrc = resolve(root, "lib", "api-zod", "src");
const generatedTypes = resolve(apiZodSrc, "generated", "types");
const indexPath = resolve(apiZodSrc, "index.ts");

// Names that collide (Zod path-param schemas in api.ts vs TS query-param types in types/)
const COLLIDING = new Set([
  "createCommercialReturnBody",
  "previewCommercialReturnBody",
  "getE11FinanzasEstadoCuentaParams",
  "listE11ConciliacionVentasParams",
  "listE11FinanzasNotasParams",
  "listE11PerfilHistorialParams",
  "listClienteRecibosE3Params",
  "listComprasProveedorParams",
  "estadoCuentaProveedorParams",
  "estadisticasProveedorParams",
  "exportarProveedorXlsxParams",
  "listProveedorPagosParams",
  "escanearRolloSalidaParams",
  "getClienteAnaliticaParams",
  "getClienteComprasParams",
  "getClienteEstadoCuentaParams",
  "getClienteEvidenciaCreditoParams",
  "getReporteSeccionParams",
  "exportReporteSeccionPdfParams",
  "exportReporteSeccionXlsxParams",
  "exportReporteVistaPdfParams",
  "exportReporteVistaXlsxParams",
  "obtenerDocumentoImpresionTicketParams",
  "bajaClienteBody",
  "crearReimpresionEtiquetasBody",
  "revisarEtiquetaRolloBody",
  "resolveE11OperacionBody",
  "getE7ClienteExportacionParams",
  "listAdminCuentaDestinoMovimientosParams",
  "exportAdminCuentaDestinoMovimientosXlsxParams",
  "getPosClienteCreditoDisponibleParams",
  "listCajaTiendaVentasParams",
  "getCajaTiendaVentasGlobalParams",
  "updateStockMinimoParams",
  "getProveedorUtilidadParams",
]);

// Build selective exports from generated/types/
const typeFiles = readdirSync(generatedTypes)
  .filter((f) => f.endsWith(".ts") && f !== "index.ts")
  .map((f) => f.replace(/\.ts$/, ""));

const nonColliding = typeFiles.filter((f) => !COLLIDING.has(f));
const colliding = typeFiles.filter((f) => COLLIDING.has(f));

// Map file name → the TS type/const names it exports (for aliasing)
const COLLIDING_TYPE_NAMES = {
  createCommercialReturnBody: "CreateCommercialReturnBody",
  previewCommercialReturnBody: "PreviewCommercialReturnBody",
  getE11FinanzasEstadoCuentaParams: "GetE11FinanzasEstadoCuentaParams",
  listE11ConciliacionVentasParams: "ListE11ConciliacionVentasParams",
  listE11FinanzasNotasParams: "ListE11FinanzasNotasParams",
  listE11PerfilHistorialParams: "ListE11PerfilHistorialParams",
  listClienteRecibosE3Params: "ListClienteRecibosE3Params",
  listComprasProveedorParams: "ListComprasProveedorParams",
  estadoCuentaProveedorParams: "EstadoCuentaProveedorParams",
  estadisticasProveedorParams: "EstadisticasProveedorParams",
  exportarProveedorXlsxParams: "ExportarProveedorXlsxParams",
  listProveedorPagosParams: "ListProveedorPagosParams",
  escanearRolloSalidaParams: "EscanearRolloSalidaParams",
  getClienteAnaliticaParams: "GetClienteAnaliticaParams",
  getClienteComprasParams: "GetClienteComprasParams",
  getClienteEstadoCuentaParams: "GetClienteEstadoCuentaParams",
  getClienteEvidenciaCreditoParams: "GetClienteEvidenciaCreditoParams",
  getReporteSeccionParams: "GetReporteSeccionParams",
  exportReporteSeccionPdfParams: "ExportReporteSeccionPdfParams",
  exportReporteSeccionXlsxParams: "ExportReporteSeccionXlsxParams",
  exportReporteVistaPdfParams: "ExportReporteVistaPdfParams",
  exportReporteVistaXlsxParams: "ExportReporteVistaXlsxParams",
  obtenerDocumentoImpresionTicketParams:
    "ObtenerDocumentoImpresionTicketParams",
  bajaClienteBody: "BajaClienteBody",
  crearReimpresionEtiquetasBody: "CrearReimpresionEtiquetasBody",
  revisarEtiquetaRolloBody: "RevisarEtiquetaRolloBody",
  resolveE11OperacionBody: "ResolveE11OperacionBody",
  getE7ClienteExportacionParams: "GetE7ClienteExportacionParams",
  listAdminCuentaDestinoMovimientosParams:
    "ListAdminCuentaDestinoMovimientosParams",
  exportAdminCuentaDestinoMovimientosXlsxParams:
    "ExportAdminCuentaDestinoMovimientosXlsxParams",
  getPosClienteCreditoDisponibleParams:
    "GetPosClienteCreditoDisponibleParams",
  listCajaTiendaVentasParams: "ListCajaTiendaVentasParams",
  getCajaTiendaVentasGlobalParams: "GetCajaTiendaVentasGlobalParams",
  updateStockMinimoParams: "UpdateStockMinimoParams",
  getProveedorUtilidadParams: "GetProveedorUtilidadParams",
};

const lines = [
  `// Auto-patched by fix-api-zod-barrel.mjs after orval codegen.`,
  `// Exports Zod schemas from api.ts first (path-param Params schemas live here).`,
  `// Then exports TS types from types/ selectively, skipping the four query-param`,
  `// Params types that share names with Zod schemas (TS2308 collision).`,
  `// Those four are re-exported under a QueryParams suffix alias.`,
  `export * from "./generated/api";`,
  ``,
  `// ── Non-colliding types ───────────────────────────────────────────────────`,
  ...nonColliding.map((f) => `export * from "./generated/types/${f}";`),
  ``,
  `// ── Colliding Params types re-exported under QueryParams alias ───────────`,
  `// (server uses the Zod versions from api.ts; these TS interfaces are for`,
  `//  the frontend – api-client-react generates its own hooks independently)`,
  ...colliding.map((f) => {
    const name = COLLIDING_TYPE_NAMES[f];
    const alias = f === "getReporteSeccionParams"
      ? "GetReporteSeccionPathParams"
      : f === "obtenerDocumentoImpresionTicketParams"
      ? "ObtenerDocumentoImpresionTicketClientParams"
      : f === "listAdminCuentaDestinoMovimientosParams" ||
        f === "exportAdminCuentaDestinoMovimientosXlsxParams" ||
         f === "exportReporteVistaPdfParams" ||
         f === "exportReporteVistaXlsxParams" ||
         f === "getPosClienteCreditoDisponibleParams" ||
         f === "listCajaTiendaVentasParams" ||
         f === "getCajaTiendaVentasGlobalParams" ||
         f === "getProveedorUtilidadParams"
      ? name.replace("Params", "ClientParams")
      : name.endsWith("Params")
      ? name.replace("Params", "QueryParams")
      : `${name}Type`;
    return `export type { ${name} as ${alias} } from "./generated/types/${f}";`;
  }),
  ``,
  `export * from "./salida-estado-presentation";`,
];

writeFileSync(indexPath, lines.join("\n") + "\n");
console.log(`[fix-api-zod-barrel] Rewrote ${indexPath} (${nonColliding.length} direct + ${colliding.length} aliased exports)`);
