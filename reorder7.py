...
# Split original_ventas by the start of Matriz to get the top cards accurately
top_cards_split = original_ventas.split('<div>\n                    <Card className="h-full">')
top_cards_wrapper = top_cards_split[0]

# remove <section className="space-y-6">\n                <h2 className="recon-heading border-b pb-2 mb-4">Ventas</h2>
top_cards_block = top_cards_wrapper.replace('<section className="space-y-6">\n                <h2 className="recon-heading border-b pb-2 mb-4">Ventas</h2>\n', '').strip()
...
matriz_block_str = "<div>\n                    <Card className=\"h-full\">" + top_cards_split[1].split('</section>')[0].strip()
...
# Note: The original abonos table ends exactly where the collection_row block's nested elements end.
# There is a <Card className="flex-1 flex flex-col"> ... </Card> inside a <div className="xl:col-span-2 flex flex-col gap-6 h-full">.
...
# Now, we also need to check new_cobranza_cards if there are any missing </div> tags.
# Actually, the destination cards look well-formed.
