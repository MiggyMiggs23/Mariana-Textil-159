...
# Extract Ventas section cards (without Matriz)
...
# Replace grid-cols-4 with grid-cols-3
ventas_cards_block = ventas_cards_block.replace('xl:grid-cols-4', 'xl:grid-cols-3')

# Remove Por cobrar from Ventas top cards
por_cobrar_card_regex = r'(<Card className="border-sidebar/10 shadow-sm recon-top-card border-dashed">.*?<div className="recon-icon-box amber">.*?<Clock className="h-6 w-6" />.*?</div>.*?<div className="recon-top-card-content">.*?<span className="recon-top-card-label">Por cobrar</span>.*?Notas de crédito al día.*?</span>.*?</Card>)'
por_cobrar_match = re.search(por_cobrar_card_regex, ventas_cards_block, re.DOTALL)
ventas_cards_block = re.sub(por_cobrar_card_regex, '', ventas_cards_block, flags=re.DOTALL)

# Extract Matriz block
...
# Extract Cobrado block (it maps over topStats)
