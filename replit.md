...
- **Vencimiento: defecto de lectura/presentación localizado, todavía sin corregir.** El diagnóstico histórico descartó D3: documento y movimiento coincidían en **2026-10-15** en ambas notas; no se volvió a consultar DB en Prompt B. Una reproducción pura con el esquema y formateador reales mostró: `2026-10-02` → coerción Date de `GetClienteNotaCreditoResponse` → JSON `2026-10-02T00:00:00.000Z` → tarjeta de crédito `01/10/2026` en `America/Mexico_City`; la impresión usa `formatDateOnlyMx` y conserva `02/10/2026`. Evidencia: `reports/prompt-b-fecha-ui-iso-tz-mexico.txt`. No lo causa la rama muerta de POS retirada por el typecheck: es otra frontera de serialización/presentación del mismo campo y queda para una entrega separada, sin alterar fechas ni coerciones aquí.
