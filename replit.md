...
El encabezado Cobrado también alimenta la banda «Cobranza del periodo» del tablero; véase «Tablero: venta y cobranza». No equivale a Contado cobrado de la identidad de ventas. La presentación separa Venta (Vendido y matriz), Cobranza (Cobrado y sus cuentas subordinadas) y Cartera (Por cobrar), sin retirar cifras ni modificar su fuente.
...
**Pendiente separado — defaults de folios por sitio (2026-09-13):** el preflight encontró defaults PostgreSQL de `99` en `entrada_folio.ultimo_folio` y `499` en `salida_folio.ultimo_folio`, mientras la regla vigente por sitio y el esquema de código parten de `0` (primer folio `1`). Estos defaults permanecen sin cambios y sin fix en este trabajo; una corrección futura evitaría que una inserción que omita el valor herede el arranque antiguo. No se corrigen como parte de la purga ni de la mejora de atomicidad; los reinicios de la futura purga serían explícitos a `0`.

**Reconstrucción atómica (2026-09-13):** `reconstruirCacheExistencias(tx)` usa la transacción recibida, incluidos los bloqueos y la reconstrucción. La purga autorizada abrió una sola transacción, pasó esa transacción a borrado y reconstrucción y propagó cualquier error para revertir todo. Sin parámetro conserva su uso independiente con una transacción propia. La evidencia de esta ejecución está en `.local/phase2-purge-evidence-20260913163631333.json`.

**Purga operativa autorizada del 13 de septiembre de 2026:** el respaldo completo verificado de `heliumdb` es `scripts/.local/backups/respaldo-antes-de-purga-2026-09-13-101833`, acompañado por el commit `b36b9a4eac11b3e630a84f775f71c597618b4f60` y el dump SHA-256 `b2e0ad1765da065cdd5d4f7dbd609eeb7c0fc31dd0b2e53c70d1360806cb22c6`. La descarga desde Drive quedó PASS, owner-only, en [el archivo de respaldo](https://drive.google.com/file/d/1Wxa6y332PLPfWNZ3sEkUlYVEZZ7XW3kb/view?usp=drivesdk), con el mismo hash `b2e0ad1765da065cdd5d4f7dbd609eeb7c0fc31dd0b2e53c70d1360806cb22c6`; los conteos y la restauración del catálogo y del esquema también quedaron verificados. El preflight de la restauración prístina quedó PASS en `.local/phase2-purge-evidence-20260913163342450.json`. El apply LIVE quedó comprometido PASS, `mode=apply`, `exitCode=0`, en `.local/phase2-purge-evidence-20260913163631333.json`; el API de source permaneció pausado durante esa línea base, y después se reinició limpio y permanece running.

- **Lista A — 33 tablas borradas dentro de una sola transacción:** `tickets`, `ticket_lineas`, `ticket_pagos`, `autorizaciones_nota`, `movimientos_credito`, `aplicaciones_credito`, `notificaciones_credito`, `sesiones_caja`, `sesiones_caja_dias`, `salidas_dinero_caja`, `cuadre_fiscal_registros`, `salidas`, `salida_lineas`, `salida_rollos`, `viajes`, `viaje_salidas`, `viaje_tickets`, `entradas`, `contenedores`, `contenedor_lineas`, `pagos_proveedor`, `aplicaciones_pago_proveedor`, `movimientos`, `rollos`, `reimpresiones_etiqueta`, `auditorias_inventario`, `auditoria_inventario_escaneos`, `auditoria_inventario_participantes`, `auditoria_inventario_snapshot`, `solicitudes_pago_dirigido`, `notificaciones_sistema`, `stock_minimo_episodios` y `sesiones`. Las 33 quedaron en cero.
- **Lista B — 7 elementos reiniciados, no borrados:** `ticket_folio` a 999; `entrada_folio`, `salida_folio`, `viaje_folio` y `auditoria_inventario_folio` a 0 por sitio; `series_consecutivo` a 1000000; y la secuencia PostgreSQL `public.contenedores_folio_seq` para que el siguiente folio sea 1. `existencias` se reconstruyó, no se vació manualmente: quedaron 30 pares, `badPairs=0`, `movimientos=0` y `rollos=0`.
- **Lista C — 18 tablas conservadas íntegramente:** `productos`, `precio_historial`, `clientes`, `cliente_documentos`, `proveedores`, `usuarios`, `ubicaciones`, `pisos`, `permisos_rol`, `permisos_usuario`, `permisos_ubicacion`, `camionetas`, `choferes`, `equipos`, `equipos_checklist`, `stock_minimo_sitios`, `stock_minimos` y `auditoria`. **`sesiones` pertenece a Lista A, no a Lista C.**

La comparación C mostró los mismos conteos y hashes antes/después en las 18 tablas; `auditoria` conservó sus 3045 renglones. Los mínimos de stock conservados en Cruces (`ubicacion_id=2`, sitio activo) son producto 1374: 2500, `2026-09-13T15:21:35.374621+00:00`, autor `1`; producto 1389: 5000, `2026-09-13T15:21:40.979699+00:00`, autor `1`; y producto 1390: 5000, `2026-09-13T15:21:56.751931+00:00`, autor `1`. Stock mínimo, equipos y permisos por ubicación son configuración y se conservaron; solo se borraron los episodios de faltantes (`stock_minimo_episodios`).

La consulta exacta posterior dejó habilitados los 11 triggers no internos (`tgenabled='O'`): `aplicaciones_credito_inmutables`, `aplicaciones_credito_validas`, `aplicaciones_pago_proveedor_append_only`, `aplicaciones_pago_proveedor_validar_insert`, `auditoria_append_only`, `auditoria_enriquecer_insert`, `movimientos_credito_inmutables`, `movimientos_credito_reversos_validos`, `pagos_proveedor_inmutables`, `reimpresiones_etiqueta_inmutable` y `ticket_pagos_inmutables`.

La comprobación disposable autenticada creó el ticket técnico con folio 1000 y luego la restauración prístina lo eliminó; el contador quedó nuevamente en 999. En las ocho pantallas comprobadas con navegador disposable, Cruces apareció Activo con sus tres mínimos; la explicación stale de Mariana/Apagado quedó corregida; el SKU restaurado `ALA-BEI` conservó precio de lista `$85.00`, costo y márgenes como `—`, y estado `Sin costo`. La evidencia está en `.local/phase2-ui-rehearsal/phase2-disposable-postpurge-proof.json` (SHA-256 `cf41530d9b3b6923c80247cccd2f9947819c61194b4e9acd4d06a8cf7dfed88c`) y `.local/phase2-ui-rehearsal/ui-clarification.json`. El intento de UI LIVE quedó **BLOCKED**, no PASS, en `.local/phase2-live-ui-evidence.json`: el puente seguro de credenciales no pudo crear un contexto autenticado, no hubo rutas comprobadas y no se registró ninguna credencial. No se infiere UI LIVE a partir del rehearsal disposable.

`pnpm run typecheck` real quedó PASS. Las cinco suites aisladas registraron 75 aserciones de escenario PASS y 0 FAIL, pero el reporte queda **WITH OBSERVATIONS**, no totalmente verde: el cleanup de inventario encontró la protección append-only de `auditoria` (`P0001`) y el cleanup de salidas encontró la FK `movimientos_salida_id_salidas_id_fk` (`23503`); ambos scripts absorbieron el fallo y terminaron con `exitCode=0`. No se confundieron esos fallos de fixtures con una falla de los escenarios ni se modificaron reglas de negocio.

La bitácora `auditoria` se conservó íntegra y por decisión expresa puede conservar referencias a documentos operativos que ya no existen. La consulta independiente postcommit PASS quedó en `.local/phase2-live-postcommit.json` (capturada `2026-09-13T16:37:38.810Z`, SHA-256 `e31f6fda34fae70656f29746748b7e368a2400902c499486e1cb2b99aa7a66f4`): confirmó A33 en cero, C18 exactas, los tres mínimos con valores/fechas/autores, switch activo, caché con `bad_pairs=0`, secuencia `1/is_called=false` y los 11 triggers `O`. La consulta independiente postrestart quedó PASS como observación en `.local/phase2-live-postrestart.json` (capturada `2026-09-13T16:44:33.894Z`, SHA-256 `45480039f54fb61adf96ceefb6ad1981dd3723871f1199c03f160f590eaf5cf2`): después del reinicio solo reaparecieron `stock_minimo_episodios=3` y `notificaciones_sistema=12`; tickets, sesiones y las otras tablas A permanecieron en cero. En C solo cambiaron `updated_at` de 72 filas de `permisos_rol` y 54 de `permisos_ubicacion`; filas, flags y autores no cambiaron, y usuarios, auditoría, mínimos, switch y equipos quedaron exactos. Esos efectos de reinicio se mantienen separados de la línea base transaccional; el API de source quedó running limpio y no se volverá a pausar. El estado global es **PURGE_COMMITTED_WITH_VALIDATION_LIMITATIONS**, no un cierre FINAL totalmente verde, porque el reporte de suites tiene observaciones y la UI LIVE quedó bloqueada.
...
- Mantener el respaldo verificado y su comprobación de restauración como artefactos operativos.
...
**Los renglones por hoja son mediciones independientes por documento:** cada constante debe llevar comentada la medida real en modo impresión después de reservar el margen físico, incluyendo encabezado, tabla, totales y pie. Salida conserva diez renglones: uno y diez productos generan una sola página con observaciones, totales y tres firmas; once se paginan como diez más uno. La Nota debe conservar su pagaré legible. Revalidar Entrada y Nota al cambiar sus dimensiones o reservas, sin asumir que las capacidades anteriores siguen cabiendo. El Ticket de contado usa tira térmica de 80 mm y no tiene una capacidad A5.

**Capacidades verificadas con margen seguro:** Nota conserva ocho renglones por copia y separa tabla de pagaré/totales. Entrada usa diez productos por hoja y veintidós filas en hojas de series; la reserva se verificó también con SKU y sitio largos que producen filas globales de 54 px, no suponiendo que todas midan 25 px. El pie completo cuenta en la capacidad de la última hoja global. Los listados de series de más de una fila van a hojas propias para no invadir esa reserva.
...
**Verificación de la implementación inicial:** 26 pruebas acotadas pasaron; se completó el chequeo de tipos de todos los proyectos y quedaron **cuatro errores únicos preexistentes**, no dos: los dos conocidos de API y dos usos de `pendiente` en Alertas ausentes de su contrato anterior. No se corrigieron. No hay errores nuevos restantes. La comprobación inicial de los cuatro estados a 402 px no quedó aprobada. Posteriormente se ejecutó el Bloque 4 autorizado y se verificaron persistencia, saldos, Historial de Abonos y Ver Reparto reales de ambas notas; esto no acredita otras pruebas de interfaz. Resultados: `reports/abonos-fifo-2026-09-15-resultado.md` y `reports/abonos-bloque4-2026-09-15-resultado.md`.

Los antecedentes están en `reports/credito-saldo-favor/verificacion-items-1-2.md` y `reports/credito-saldo-favor/resultado-2026-09-15.md`. Sus verificaciones no se acreditan como ejecutadas en esta entrega. No se reabre la matriz de legibilidad PDF/XLSX cerrada ni la corrección cancelada de los dos errores de tipos.
...
- El diagnóstico actual, ejecutado en READ ONLY mediante el pool de la aplicación, confirmó D1 y D2 para los abonos 46 y 45 de los clientes 6 y 7 y las notas 1004 y 1005 de Cruces. Ambos abonos se guardaron a medianoche UTC, equivalente al día anterior en México, aunque su captura real fue posterior a la autorización de la nota. Al preceder al cargo marcado con `preventImplicitFavor`, quedaron sin aplicación. Consultas y salidas textuales: `reports/abonos-fifo-2026-09-15-diagnostico.md`. Este diagnóstico no investiga el caso antiguo posiblemente purgado.
- D3 no se confirmó: en ambas notas, documento y movimiento vencen el **2026-10-15**. No se modificaron vencimientos.
- **Fechas efectivas de abonos:** toda fecha que viaje entre navegador y servidor lleva hora y desplazamiento explícitos; el servidor nunca infiere la zona de una fecha pelona. El diálogo usa una función compartida de Ciudad de México para abono normal, dirigido y vista previa. Fecha vacía en el diálogo envía null y conserva el instante del servidor. No se modifican instantes históricos.
- La simulación histórica de solo lectura encontró **dos notas cuyo saldo se reduciría y cero que quedarían pagadas** al retirar indiscriminadamente el veto. No se aplicó esa simulación ni se retiraron las marcas. Las correcciones expresamente autorizadas del Bloque 4 se realizaron mediante reversos y recapturas; no mediante neteo ni alteración de los originales.
- Bloque 4 **ejecutado y verificado tras autorización expresa**: reversos 47/48 de los abonos 46/45 y recapturas 49/50 con los instantes originales exactos. Deudas $5,750.00 / $7,022.00; favor cero en ambos. Se conservó el veto histórico. El día 14 conserva $25,000 y el 15 quedó en cero neto por estas cuatro operaciones; no se cambió Cobrado. Evidencia: `reports/abonos-bloque4-2026-09-15-resultado.md`. No volver a ejecutar el plan.
...
- **Regla vigente, que sustituye expresamente la aplicación manual anterior:** todo dinero recibido consume primero la deuda elegible por FIFO; solo el remanente genera favor. Al autorizar una nota nueva, el favor disponible se aplica automáticamente hasta el menor importe. No hay casilla ni importe manual que pueda eludir el cálculo del servidor o el límite duro.
...
- Las restricciones existentes de evidencia no cubren todos los casos de favor liberado por reversos ni los orígenes AJUSTE. No se deben convertir sus filas históricas en fuente de saldo ni ofrecer como autorizable una aplicación que no pueda documentarse exactamente. Estos casos quedan como bloqueos explícitos, no como autorizaciones financieras reales aprobadas.
- Queda un título incorrecto, **Cobros de periodos anteriores**, aunque su subtítulo ya dice Abonos a notas. Se reportó como faltante en revisión, sin corregirlo después de la verificación, conforme a la instrucción de detener las correcciones al encontrar fallos.
...
- **Cobranza del periodo**, en una banda independiente entre ventas y señales operativas, consume `useSharedCuentasDestino` en `artifacts/mariana-textil/src/hooks/use-shared-cuentas-destino.ts`, igual que Cuentas Destino. Ambas usan el mismo endpoint y la función existente `getDestinationAccounts` de `artifacts/api-server/src/lib/admin-analytics.ts`. No hay un cálculo financiero alternativo en el navegador ni una segunda implementación SQL.
- El total y los grupos De ventas del periodo, Abonos a notas y A cuenta, sin aplicar vienen de `encabezado.cobrado`. Los enlaces reutilizan el detalle canónico conservando periodo, sitio y fuentes. Los filtros públicos de abonos incluyen sus reversos mediante la expansión existente del servidor. Carga o error de cobranza se muestran en su propia banda, sin ocultar las tarjetas de ventas.
...
La revisión estática de `listarSesionesCajaHistorial` y `buildCorteCaja`, en `artifacts/api-server/src/lib/pos.ts`, confirma que el efectivo esperado usa **fondo inicial + pagos EFECTIVO de tickets − salidas de efectivo**. Ese camino no incorpora los ABONO de `movimientos_credito`. Es una omisión pendiente para la conciliación de efectivo, **no corregida aquí**; no se ejecutó ningún cierre ni cuadre.
...
Se ejecutaron lecturas reales en transacciones `REPEATABLE READ READ ONLY`, sin login ni escrituras. Las comparaciones antes/después conservaron las cuatro tarjetas existentes y las 125 hojas numéricas de Cuentas Destino entre Cruces y Global. Se comprobaron identidad de ventas, grupos de fuentes y suma/conteo de todas las páginas del detalle. Los movimientos 43–50 y las huellas del veto consultado se conservaron. Las tablas y salidas están en `reports/prompt-c-readonly-2026-09-15-after.md` y su JSON.

Codegen terminó sin diferencias. El chequeo completo de tipos requirió completar los proyectos detenidos por el primer fallo; permanecen los cuatro diagnósticos preexistentes (dos API y dos Alertas), no se consideran resueltos. La validación visual real sigue **no aprobada** por falta de sesión autenticada; cualquier comprobación con respuestas interceptadas acredita solo presentación, no acceso real ni una operación financiera. El resultado final y los límites de capturas se documentan en `reports/prompt-c-resultado.md`.