...
Antes de cargar los clientes reales, cambiar **una sola línea** en `artifacts/api-server/src/lib/test-reset/manifest.ts`: `export const PROTECT_CUSTOMERS = true;`. Se debe liberar esa versión antes de importar la lista. El servidor preserva todos los clientes y el diálogo muestra automáticamente la protección; no hay que cambiar otra bandera de interfaz. El cliente interno se conserva en ambos modos. Ambos modos deben quedar comprobados en la base desechable.
