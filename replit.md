...
**Automatización de `TEST_DATABASE_URL` — pendiente, no implementada:** es la conexión a la base aislada de cada corrida, no la conexión operativa ni una contraseña que deba inventar el propietario. El objetivo solicitado es que el aprovisionamiento de la rama desechable la obtenga e inyecte únicamente al proceso de pruebas, verifique la identidad y elimine la rama al terminar. Actualmente las suites aún requieren la variable explícita; no existe un aprovisionador automático en el repositorio. Antes de ejecutar suites mutantes se debe resolver su incompatibilidad con la población exclusivamente desde el seed autorizado: algunas crean fixtures adicionales y otras crean usuarios. No ejecutar esas suites ni ampliar el seed por cuenta propia.

**2026-09-12 — deuda pendiente acumulada:** implementados el renombre a `documentosPendientes` y la resolución de documentos de bolsas/piezas en Movimientos, con validación de documento en consumo de venta diferida. Codegen reproducible sin diferencias; tanda contractual sin DB: 69 aprobadas y 1 omitida. Verificación incompleta: typecheck global falla por TS6059/TS5097 en scripts; el test de trazabilidad falla al extraer una función con `export`. No se corrigieron esos fallos sin autorización. No se ejecutaron suites DB ni se confirmó visualmente el contenido de Movimientos: la vista automática requiere autenticación. El aprovisionamiento automático de pruebas sigue pendiente.
