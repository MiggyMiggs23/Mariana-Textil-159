...
**2026-09-15:** se completó la visibilidad del saldo a favor del cliente incluso en cero, el saldo pendiente actual en notificaciones y la proyección de autorización al seleccionar saldo a favor. Se ejecutaron codegen, typecheck completo, contratos/pruebas puras, comprobaciones de navegador con respuestas simuladas y una prueba de la consulta real de Cobrado en solo lectura. Persisten fallos y cobertura bloqueada: no equivale a aprobación integral. Ver `reports/credito-saldo-favor/verificacion-items-1-2.md` y `reports/credito-saldo-favor/resultado-2026-09-15.md`.
...
- La comprobación del 2026-09-15 mediante el pool configurado del entorno volvió a encontrar cero ABONO; no hay publicación activa en este proyecto. Falta identificar la instancia y el folio del caso original. No afirmar que la causa del caso concreto está confirmada ni resuelta.
- Los cambios de corrección del Bloque 2 preparados antes del diagnóstico se retiraron íntegramente. No se ejecutó su inicializador ni se creó su tabla; se comprobó su ausencia en solo lectura. La corrección financiera sigue esperando diagnóstico y propuesta comunicados al propietario.
...
- La prueba en memoria de `clientes-aging.test.ts` no basta como evidencia financiera. Se ejecutó además `verify-favor-cobrado-readonly.ts`: llama a la consulta real `getDestinationAccounts` con CTEs `VALUES` en una transacción READ ONLY y verifica conservación global/por periodo al recibir y aplicar saldo a favor, sin otro ABONO. Pasó con 24 consultas y cero escrituras. **No verifica el filtro por sitio ni el flujo de escritura.** La revisión conserva el hallazgo de que aplicar saldo puede cambiar Cobrado filtrado por sitio; no se acredita la regla completa.
- Queda pendiente la coherencia entre cancelación/reaplicación y el límite de aplicaciones de la base. La interfaz ya recalcula la autorización cuando el saldo seleccionado resuelve el exceso de límite; el servidor conserva su validación transaccional. Esto no acredita haber autorizado una nota real.
