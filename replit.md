...
- **Verificación ejecutada en Prompt O:** reproducción antes/después con nota real en READ ONLY; esquemas generados, render real de `ClienteNotaCredito` mediante SSR, bordes de mes/hoy/año bisiesto, cálculos de días y XLSX real en memoria aprobados. Se ejecutaron 17 pruebas de contrato de impresión y se confirmó que sus archivos productivos y POS no cambiaron. Typecheck recursivo completo en cero, codegen estable y builds aprobados. La vista previa real mostró login/401 sin sesión; **la nota real y su impresión en navegador autenticado no se verificaron**. La API nueva no se ha activado bajo la prohibición de escrituras. Detalle y límites en `reports/calendar-due-date/entrega.md`.
