### Versión activa: directorios y sitios SIN RFC nuevo de proveedores (26/09/2026)

Con autorización literal del propietario «Osea activa todo menos ese, ese cambio no lo quiero», MAIN activó **API y frontend** `dist-directorios-sitios-20260925-sin-rfc`; «ese cambio» es el nuevo RFC de proveedores, rechazado. No se añadió la columna, no se ejecutó DDL ni migración, y no se liberaron cambios ajenos de HEAD. Permanecen los directorios de Clientes y Proveedores, sus periodos/conteos, historiales, selectores de crédito según sitio y únicamente las simplificaciones aprobadas 1, 2, 6, 7 y 8. El RFC ya existente de **clientes** se conserva.

MAIN validó los TOML administrados y los hashes protegidos, reinició una vez cada workflow administrado, observó API PID 20373 en inspection boot sin inicializadores, frontend HTTP 200 con hash de `index.html` `7f7650f603302b32a985eac9fbb64e0fd4e3897c17093f307f912dcede21b926` y `/healthz` HTTP 200. Capturas READ ONLY antes/después del `DATABASE_URL` del proceso efectivo: 108 tablas, misma base y esquema; ningún cambio en conteos ni hashes de contenido. Evidencia, manifiesto, hashes y límites: `reports/directorios-sitios-20260925/activation-sin-rfc/resultado.md`. No se afirma prueba visual con sesión autenticada.

### Versión anterior: directorio de clientes (histórico, ya sustituido)

API y frontend sirvieron `dist-clientes-lista-20260925`, activado tras la autorización «Sí, activar sin modificar datos». MAIN confirmó hashes de arranque e inspection boot sin inicializadores. Los accesos rápidos aparecen en el orden solicitado: A–Z, Más movimientos, Última actividad, Deuda pendiente; el criterio inicial sigue siendo última actividad descendente. Evidencia, contrato, procedencia y pruebas: `reports/clientes-lista-20260925/`. El listado paginado mantiene intacto GET /clientes para consumidores operativos. La actividad cuenta documentos comerciales procesados una sola vez y abonos no revertidos; el conteo depende del período, mientras la última actividad considera todo el historial. Deuda y saldo a favor conservan la proyección canónica; actividad respeta alcance y permiso financiero. Las capturas READ ONLY antes/después de las 108 tablas no detectaron diferencias de conteos, contenido ni esquema. No se modificó ningún cliente ni movimiento, ni se aplicó DDL. No hace falta índice adicional según las mediciones de esa versión. Salud HTTP 200 y listado anónimo HTTP 401; no se creó sesión para medir navegador autenticado, por lo que no se afirmó una garantía de carga completa menor a dos segundos.
...
### Trabajo nocturno 25/09/2026 — estado canónico de apertura

La autorización textual y los límites por tarea están en
`reports/trabajo-nocturno-20260925/INFORME.md`. MAIN aplicó a la base efectiva
de la API, con resultado `COMMITTED`, SQL E5 y E11; activó
`dist-night-task12` API/UI (PID API 39225, web 39258), mantuvo
`API_INSPECTION_BOOT` y observó health HTTP 200 a
2026-09-25T05:09:38Z; SHA-256 API
`a20509b6a2b050262a50dc3852f9a76f94aac8726f6ee16efad594e4be23b206`.
E5 dirigido/retenido y preparación ContadorA están ON; ADMIN conserva
aplicación, ContadorF es el default de CONTADOR sin A expresa. No se asignó
ningún perfil de aplicación: inventario preflight cero cuentas CONTADOR/cero
perfiles. E5 devolución, Fondo, E12 y atribución histórica siguen cerrados.
La API de inspección pausa inicializadores, backfill y monitor, pero no es
globalmente de solo lectura.
...
El plan no concede permiso de purga. No alterar UI previamente aprobada.
...
### Estado servido de Tanda E — antecedente histórico del 23 de septiembre de 2026
...
- En aquella fecha este estado no abría Fondo/E10, E12, E5 monetario, retiro dirigido/retenido, atribución histórica E1 ni purga. La apertura autorizada del 25/09 sustituye únicamente aquellos cierres E5/E11; véase «Trabajo nocturno» abajo. Informe histórico: `reports/tanda-e-20260923/INFORME.md`.

### Liberación simple temporal — vigente desde el 23 de septiembre de 2026

Fuente inicial: `reports/autorizacion-procedimiento-simple-y-liberacion-e3-tanda-b-20260923.txt`. Esta regla rige **solo mientras nadie opere el sistema y todos sus datos sean de prueba**. Antes de cualquier escritura en la base de la API sigue siendo obligatoria una autorización escrita del propietario. Esa fuente autorizó aplicar E3 y los cinco SQL de Tanda B, ya instalados y no repetibles. Autorizaciones posteriores abrieron remate/precio/borrado individual, E4, E9 documental y lectores E7. La autorización **más reciente** `reports/trabajo-nocturno-20260925/autorizacion-propietario.txt` abrió pago dirigido, cobro retenido y perfiles ContadorF/A; sus SQL ensayados se aplicaron con COMMIT a la base efectiva, E5 general y preparación A están ON en fuente y candidato API/UI activo. Sustituye únicamente el cierre de E5 monetario, retiro dirigido/retenido y preparación A declarado en los informes anteriores. **Devolución E5, Fondo/E10, E12, atribución histórica E1 y purga general siguen cerrados.** La atribución financiera E7 de solo lectura permanece distinta de la atribución histórica. Alcance, límites E2E y evidencia: «Trabajo nocturno» y `reports/trabajo-nocturno-20260925/INFORME.md`; ninguna otra puerta se presupone habilitada.

### Excepción temporal: reinicio del sistema de pruebas

Mientras el propietario siga usando exclusivamente datos de prueba, se autoriza **solo** la acción explícita del botón de administración «Borrar datos de prueba». Es una excepción estrecha a la prohibición general de borrar datos operativos; no cambia las reglas que prohíben borrados en operaciones ordinarias ni habilita otras purgas. La acción exige ADMIN y escribir `BORRAR`; el conjunto fijo de tablas y filas se enumera en código, nunca lo elige el usuario. Se ensaya exclusivamente sobre una base PostgreSQL desechable antes de habilitarla. No ejecutar el reinicio sobre la base de la aplicación como parte de desarrollo, pruebas, instalación o liberación.

El reinicio conserva **todos los clientes**, proveedores, catálogo y precios vigentes, usuarios, permisos, sitios y pisos, camionetas, choferes, equipos y el resto de la configuración. Borra datos operativos de prueba: existencias, rollos, movimientos; ventas, tickets, notas, entradas, salidas y viajes; pagos, abonos, crédito y aplicaciones; sesiones y movimientos de caja; series y registros de etiquetas impresas; checklist de equipos; auditorías, alertas y notificaciones; bitácora general e historial de cambios de precios; y actividad de fondo, custodia y conciliación, incluida su evidencia. Los saldos de prueba del fondo quedan en cero. Los folios documentales se reinician a cero y la serie siguiente de rollo comienza en `10000001`. Las configuraciones financieras se conservan. Cualquier tabla nueva o no clasificada debe bloquear el reinicio hasta quedar clasificada en la lista fija; no se amplía el borrado por conjetura.

Cada ejecución conserva, fuera de las tablas que el reinicio vacía, un registro protegido de quién lo ejecutó y cuándo. Al confirmar el éxito, el sistema cierra las sesiones; a quien inició el reinicio le informa claramente que debe volver a iniciar sesión y lo lleva al login, sin dejar que un error de autorización rompa la pantalla.

**Protección de clientes solicitada por el propietario:** `export const PROTECT_CUSTOMERS = true;` en `artifacts/api-server/src/lib/test-reset/manifest.ts`. El botón nunca borra clientes: conserva sus datos, límites y plazos de crédito, contactos, domicilios, estado activo/inactivo y documentos de identificación asociados (`cliente_documentos` contiene INE y referencias de almacenamiento, no notas financieras). El diálogo refleja automáticamente la protección. Se eliminan notas/tickets, pagos, abonos, aplicaciones y todo el historial financiero; el caché `saldo_credito` vuelve a cero. No queda deuda ni saldo a favor, pero el límite autorizado permanece intacto. Ensayo real exclusivamente desechable y candidato: `reports/test-reset-protected-customers-20260925/README.md`. No se autoriza ejecutar el borrado en la base de la aplicación.

**Retiro obligatorio:** cuando el propietario empiece a operar de verdad, se eliminan el botón y todo el código exclusivo del reinicio, incluyendo su endpoint, lista destructiva, acción de datos y pruebas específicas. No basta con ocultarlo o apagar una bandera, conservar el endpoint o dejar una purga ejecutable. La excepción termina en ese momento y vuelve a regir íntegramente la política ordinaria: las tablas de operación no admiten borrado/reset.

Esta excepción no autoriza arrancar ni reiniciar la API o ningún workflow, ejecutar inicializadores, instalar DDL sobre la base de la aplicación, ni alterar el bundle que está sirviendo. La restricción de arranque, instalación e identidad de bundles de abajo sigue vigente; una liberación requiere su propia autorización explícita y todos los pasos de ensayo aislado y verificación que correspondan.

**Activación autorizada del 25 de septiembre de 2026:** el propietario respondió «Sí, activar sin borrar datos» a la autorización separada de activar el botón y reiniciar la API. Se activó exclusivamente este cambio sobre las fuentes del bundle retenido, con arranque inspection y exclusión de procesos. La autorización no incluyó ejecutar el reset; no se instaló DDL ni se borraron datos. La comparación de las 107 tablas antes/después no detectó diferencias de conteos ni contenido. Evidencia y procedencia: `reports/test-reset-20260925/README.md`. La excepción temporal, el futuro modo de protección de clientes y el retiro obligatorio siguen vigentes.
...
La liberación simple termina cuando el propietario decida comenzar a operar con datos reales. Entonces se retira íntegramente el botón de reinicio temporal descrito arriba y se restablece el procedimiento completo. Esta regla **no autoriza una purga general ahora** ni la carga/destrucción de datos reales.
...
- Las tablas operativas no usan DELETE. En inventario, las correcciones usan un inverso que referencia el original solo si el rollo sigue en el mismo sitio, cantidad y estado en que lo dejó el movimiento; de lo contrario se bloquea claramente y se requiere un ajuste nuevo con motivo.
...
- En traslado normal: `ARMANDO → EN_TRANSITO`, `ARMANDO → CANCELADA`, `EN_TRANSITO → CANCELADA` y `EN_TRANSITO → RECIBIDA`. Un usuario autorizado puede cancelar un traslado en `ARMANDO` o `EN_TRANSITO` conservando las comprobaciones de permiso, origen/destino y la restricción vigente de `CAJA`; la cancelación en tránsito devuelve atómicamente **todos los rollos al origen original** mediante movimientos compensatorios trazables y nunca borra el historial. Si el origen tiene pisos activos, el operador debe elegir explícitamente el piso de retorno en el diálogo; no existe un piso original autoritativo que pueda restaurarse automáticamente. Si no hay pisos activos, el campo se omite. `RECIBIDA` y `ENTREGADA` no se cancelan: una corrección posterior por inverso de inventario requiere que el rollo conserve sitio, cantidad y estado dejados por el movimiento; de lo contrario se bloquea claramente y se realiza un ajuste nuevo motivado. En venta a cliente se aplican el ciclo, cancelación y comprobaciones de entrega descritos en «Salidas para venta a cliente».
...
Mecánicamente reusan lo que ya existe: estado `BAJA`, movimiento `AJUSTE_NEGATIVO` por la cantidad completa en negativo, y `revertirMovimiento` para corregir un error únicamente si el rollo conserva sitio, cantidad y estado dejados por ese movimiento; de lo contrario, bloqueo claro y ajuste nuevo motivado. No hay tabla ni ciclo de vida propio.
...
**Límites históricos de construcción E3, sustituidos para la liberación por autorizaciones posteriores:** código, interfaz y permisos nuevos permanecían apagados hasta autorización separada de liberación. El flujo dirigido sin ADMIN exige exactamente el saldo pendiente de las notas indicadas (P6); recibido pendiente no reduce deuda ni crea favor disponible. Esta construcción no autorizaba acceso a la base de la API ni reinicios. El cierre histórico del flujo dirigido se sustituyó **solo** por la autorización del 25/09 documentada arriba; devolución sigue cerrada.
...
**Sesión solo cuando corresponde a dinero físico en efectivo.** Su soporte exige `sesionCajaId` explícita, abierta y del mismo sitio de origen, con `CAJA_FISICA`; nunca se fabrica ni se deduce una sesión. Una transferencia exige cuenta bancaria válida y ninguna sesión de caja. Una corrección o una operación de crédito sin dinero tampoco imputa sesión de caja. **Estado histórico E1:** captura nueva de efectivo de crédito deshabilitada entonces; la apertura E5 del 25/09 habilitó recepción dirigida/retenida bajo guardas específicas, **no** devoluciones físicas en efectivo. La migración original no incluía las guardas independientes: se agregaron en intervención posterior autorizada. No se infiere de su sola presencia captura habilitada ni corte corregido. SQL y ensayo históricos: [resultado-y-propuesta.md](reports/e1-guardas-temporales-2026-09-18/resultado-y-propuesta.md); [resultado-operativo.md](reports/e1-guardas-operativa-2026-09-18/resultado-operativo.md).
...
**P9/P10 — conflicto RESUELTO; apertura E11/E5 del 25/09:** toda cuenta CONTADOR sin asignación A expresa funciona como ContadorF y pierde acceso fuera de lo facturado, así como la capacidad de registrar pagos a proveedor o pagos de cliente. Sustituye «CONTADOR ve todo lo financiero» y la conservación de sus permisos de pagos; no se conservan permisos u overrides que contradigan ese alcance. El selector ADMIN A/F está habilitado con motivo, revisión e historial; en la base de aplicación había cero CONTADOR y cero perfiles en el preflight y no se declara asignación posterior. El propietario elige manualmente las personas ContadorA. **Preparación E5 de A abierta**, separada de la aplicación reservada a ADMIN; devolución E5 cerrada.
...
**CONTADOR — perfiles E11 vigentes:** ContadorF ve solo facturas, clientes facturados y ventas facturadas, con la conciliación definida en P9. No ve lo financiero no facturado, Fondo ni pagos a proveedor, ni registra pagos de cliente o proveedor. ContadorA obtiene la lectura financiera saneada definida por E11 y puede preparar destinos E5 sin efecto contable; solo ADMIN autoriza/aplica. No toca mercancía ni administra el sistema. Sin asignación A expresa se resuelve F. Los movimientos nuevos del ledger de crédito E1 exigen rol operativo autorizado: CONTADOR, SISTEMAS y BODEGA no los producen; E1 no crea un nuevo rol contable ni amplía esos permisos.
...
**Decisión vigente del propietario — reversión de movimientos de inventario:** un movimiento de inventario solo puede revertirse si el rollo sigue en el mismo sitio, cantidad y estado en que lo dejó ese movimiento. En caso contrario debe bloquearse con un mensaje claro y realizarse un ajuste nuevo con motivo. El propietario acepta que esto bloquee correcciones legítimas: deberán deshacerse los movimientos en orden inverso o realizarse un ajuste. La condición se comprueba por sitio, cantidad y estado, no por la mera existencia de un ID posterior; no se inventa procedencia cuando falta evidencia.

Esta decisión sustituye la propuesta del Bloque 2 y sus decisiones pendientes sobre reversibilidad; ya no se exige permitir una corrección independiente que no cumpla la condición anterior. Es una política de inventario, no modifica los inversos del ledger financiero ni E10. Su registro no acredita implementación de las diez vías ni autoriza su corrección automática. En este alcance quedan excluidas la vía 3 (venta parcial, catálogo confirmado con cero productos activos elegibles) y la vía 10. Tampoco se reanuda el Prompt P.
...
**Retenidos, regla de devolución aún cerrada:** un cobro retenido sólo sería devolvible mientras permanezca íntegro y nunca aplicado. No se fabrica un ABONO ni un REVERSO para devolverlo. La preparación exige evidencia positiva de origen. En esta fase histórica E2 no se habilitó captura ni conversión E5; la apertura E5 del 25/09 sustituyó ese cierre de captura, **no** habilitó devolución ni rellenó históricos.
...
atribución financiera de solo lectura; por sí solas no reanudaban operaciones E5 (abiertas después el 25/09), atribución
...
- E9 documental habilita envío, conteo, autorización de custodia e
  ...
  y atribución financiera de solo lectura; por sí solo no abrió operaciones E5
  (habilitadas después, separadamente, el 25/09), atribución
