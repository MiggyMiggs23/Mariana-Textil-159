...
- **Historial de precios sin movimientos — decisión de negocio y detención (2026-09-18):** el propietario permite borrar un producto sin movimientos que solo tenga historial de precios. Esto sustituye únicamente el estado «pendiente de decisión de negocio». También exige detenerse y reportar, antes de proponer cómo, si el borrado obliga a eliminar o dejar huérfanos renglones de `precio_historial`. **Impedimento detectado por lectura estática:** el esquema declara `producto_id` obligatorio con referencia a `productos.id`, y la purga bloquea esas referencias. No es compatible borrar físicamente el producto conservando intactos sus renglones con la misma referencia. **Punto detenido, sin propuesta de solución:** se conserva el bloqueo operativo y no se elimina ningún renglón de `precio_historial`; no se resuelve esa incompatibilidad por cuenta propia. No se consultó ni modificó la base. Detalle en `reports/prompt-u-respuestas-2026-09-18.md`.
...
- **Decisiones adicionales:** se confirma la prohibición de facturar ventas con líneas por metro y la confirmación de borrado con usuario y contraseña ADMIN, actualizadas en sus secciones. El borrado de producto con solo historial de precios tiene decisión de negocio favorable pero queda **detenido por la relación obligatoria con ese historial**, sin propuesta de solución. Los movimientos #51, #52 y #53 se conservan «Sin sitio determinado»; **los 69 vetos de acción por rol siguen pendientes**, sin modificación de permisos.
...
Las respuestas textuales, referencias al texto sustituido, los conflictos anteriores y el impedimento del historial de precios se conservan en el informe citado. No se dan por contestadas otras preguntas del plan ni se altera el estado inactivo de los flujos preparados.
...
1. **Remate — entrega nueva, no construir todavía.** Solo ADMIN puede marcar rollos específicos como «remate», con motivo obligatorio. Solo esos rollos pueden venderse por debajo del costo; el cajero no puede autorizarlo por su cuenta. La venta queda señalada como remate y su pérdida aparece en los reportes de utilidad. Fuera de esta excepción futura, vender por debajo del costo sigue bloqueado. No se extiende la marca a todo un producto ni se declara habilitada.
...
**Conflictos/tensiones reportados, sin resolver:** la exclusividad ADMIN para marcar remate debe conciliarse con la regla vigente que configura restricciones de acciones en la matriz y rechaza techos inmutables por rol; no se decide una excepción ni se resuelven los 69 vetos pendientes. La purga completa futura entra en tensión con la prohibición de DELETE operativo y la conservación del histórico financiero: las purgas excepcionales anteriores no autorizan otra ejecución. Se conservan las reglas vigentes, sin habilitar excepciones ni definir cómo sortearlas. La nueva purga no levanta la detención del borrado individual de productos con historial de precios.

El registro histórico de confirmación bajo costo no se reescribe como si ya hubiera sido un bloqueo. Los conflictos anteriores P9/P10 y P12 siguen pendientes. Estas decisiones no dan por terminada ninguna entrega ni abren flujos operativos.
