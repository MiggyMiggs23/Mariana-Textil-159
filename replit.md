...
### Arranque de inspección del entorno de desarrollo

Tras un reinicio automático del entorno, se habilitó `API_INSPECTION_BOOT=1` **solo en desarrollo** para cargar el código sin escrituras de arranque. Inicializadores de esquema, backfill de compras y monitor automático de stock mínimo quedan pausados con esta opción. La API y el frontend están sirviendo; la opción no deshabilita las operaciones normales de los usuarios ni cambia sus permisos. **No implica que los GET autenticados sean libres de escrituras:** pueden renovar la sesión. El arranque normal de producción no cambió. No retirar esta opción ni ejecutar el mantenimiento como parte de otra entrega que prohíba escrituras; su reactivación debe ser explícita.