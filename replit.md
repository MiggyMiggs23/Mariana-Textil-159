...
**Inventario vivo posterior de Prompt H:** la captura de la base actual documenta **14 triggers no internos vivos**. Este es el conteo vigente para diagnóstico; no se modificó ningún trigger.
...
El primer intento de preflight **NO-GO por freshness** es únicamente histórico y se conserva como puntero en `reports/prompt-h/previous-live-api/resultado-bloques-2-3.md`; la renovación inmediatamente anterior quedó preservada en `reports/prompt-h/history-paused-213056/`. Ninguno es un bloqueo actual. El estado vigente del commit es `COMMITTED_POSTCOMMIT_READ_PASS` según `.local/prompt-h-authorized-purge-status.json`. Después del commit, MAIN reinició la API y el frontend y pausó la API tras el drift post-arranque. Al solicitar el propietario recuperar su acceso, la API se reanudó el 15 de septiembre de 2026 a las 22:27, Ciudad de México; ambos servicios están ejecutándose y `/api/healthz` respondió HTTP 200. No se cambiaron credenciales. `public.contenedores_folio_seq` quedó conservada sin reset ni nuevo objetivo; su decisión empresarial futura permanece abierta, pero no bloquea la purga completada.
...
**Inventario vivo vigente de Prompt H:** la captura preflight y la verificación
...
**Verificación de esta sesión:** typecheck completo con cero diagnósticos por paquete y etapa; codegen repetido con hashes idénticos; 25 pruebas backend/contrato/arranque, seis de navegación y tres de fixtures aprobadas. Se comprobaron a 402 y 1280 px tres movimientos reales de un respaldo histórico, sin restaurarlo. Por separado se comprobaron casos sintéticos de reparto múltiple, ajustes con ambos signos, nombres largos y cantidades grandes; no se presentan como operaciones reales. La base actual no conserva movimientos de crédito y no se abrió una sesión autenticada. Evidencia y límites: `reports/prompt-f/resultado.md`.

### Arranque normal de la API: escrituras esperadas y mantenimiento

**Estado vigente, 2026-09-16:** se retiró `API_INSPECTION_BOOT` del entorno de desarrollo con autorización del propietario. API y frontend arrancaron con su comportamiento normal; todos los inicializadores completaron, el backfill de compras terminó con `inserted: 0` y se reactivó el monitor de mínimos de stock.

**Regla de trabajo confirmada por el propietario:** el arranque normal de la API ejecuta escrituras de inicialización reproducibles e idempotentes; no son contaminación ni datos artificiales de prueba. Una restricción de «no escribir en la base» sobre el trabajo de una entrega no debe extenderse a los inicializadores normales de un reinicio de servicio. No es posible prometer simultáneamente un reinicio normal y cero escrituras absolutas de arranque. En Prompt F se pausó el mantenimiento al interpretar esa restricción de forma absoluta; no repetir esa pausa por la misma contradicción. Esta excepción no autoriza escrituras de negocio, usuarios/sesiones de prueba, restauraciones ni modificaciones manuales ajenas al arranque. Si en otro trabajo se prohíben expresamente también las escrituras de inicialización, no ejecutar un reinicio normal bajo la afirmación de que es de solo lectura.
...
## Prompt Q — diseño de atribución; sin implementación

Documento de decisión para el propietario y su socio: `reports/prompt-q-diseno-atribucion-sitio.md`. La ausencia de atribución explícita de sitio y sesión en `movimientos_credito` es la causa raíz común de tres defectos abiertos: abonos omitidos del corte, asimetría de cobranza por sitio y pérdida de abonos sin ticket en el estado de cuenta restringido. Resolverla también exige distinguir recepción física, aplicación a una nota y corrección contable; una columna por sí sola no prueba efectivo.

Reglas de negocio confirmadas en el encargo: cada tienda cobra sus propias notas; un abono con ingreso físico de efectivo exige caja abierta y entra al corte, mientras una transferencia no exige caja. Son requisitos para el diseño, no afirmaciones de que estén implementados. El FIFO y la proyección siguen siendo globales; el documento expone el conflicto posible entre su reparto y la regla de notas propias, sin cambiar ninguno.

**Prompt P permanece detenido por este bloqueo, por instrucción expresa del propietario: Grupo 1 sin cerrar ni aprobar; grupos 2, 3 y 4 sin continuar.** Las alternativas del documento son propuestas pendientes de decisión, no autorización para implementarlas.
...
Solo se escribió documentación. No hubo cambios de aplicación, contratos, esquema o datos, ni migraciones, usuarios, sesiones de prueba, reinicios, pruebas funcionales, typecheck o codegen. El documento no cierra ninguno de los tres defectos.