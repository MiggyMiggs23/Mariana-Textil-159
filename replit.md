...
El encabezado Cobrado también alimenta la banda «Cobranza del periodo» del tablero; véase «Tablero: venta y cobranza». No equivale a Contado cobrado de «POS y Caja — regla contable vigente». Véase también «Cobrado: dos definiciones sin reconciliar», cuya decisión corresponde al propietario. La presentación separa Venta (Vendido y matriz), Cobranza (Cobrado y sus cuentas subordinadas) y Cartera (Por cobrar), sin retirar cifras ni modificar su fuente.
...
- En `artifacts/api-server/src/lib/admin-analytics.ts` existen **seis copias manuales completas** del predicado de `accountedDocumentPredicate`, cuya declaración canónica está en `artifacts/api-server/src/lib/accounted-document.ts`.
- **Cinco** están en `getSalesSummary`: filtro del CTE `lines` para margen, suma `ventas`, suma `subtotal`, suma `iva` y conteo `tickets`. **Una** está fuera de esa función, en `getSessionMargin`, para el margen por sesión. No se confunden usos del helper con copias manuales. Su unificación sigue pendiente en una entrega propia con pruebas; no se realizó aquí.
...
- **Lista B — 7 tablas, más una secuencia separada:** la lista declarada en `scripts/src/purge-operational-phase2.mts` contiene `ticket_folio`, `entrada_folio`, `salida_folio`, `viaje_folio`, `auditoria_inventario_folio`, `series_consecutivo` y `existencias`. El resultado histórico reinició los seis contadores: ticket a 999; entrada, salida, viaje y auditoría a 0 por sitio; series a 1000000. Además reinició `public.contenedores_folio_seq` para que el siguiente folio fuera 1; **esa secuencia no es una tabla**. `existencias` se reconstruyó, no se vació manualmente: el reporte registró 30 pares, `badPairs=0`, `movimientos=0` y `rollos=0`. No se consultó nuevamente el estado de esas tablas en esta auditoría.
...
El resultado histórico de la purga registró conservación íntegra de `auditoria`, incluidas referencias a documentos operativos retirados. La evidencia postcommit está en `.local/phase2-live-postcommit.json` (capturada `2026-09-13T16:37:38.810Z`, SHA-256 `e31f6fda34fae70656f29746748b7e368a2400902c499486e1cb2b99aa7a66f4`): reportó A33 en cero, C18 exactas, mínimos con valores/fechas/autores, switch activo, caché con `bad_pairs=0`, secuencia `1/is_called=false` y 11 triggers `O`. La observación postrestart está en `.local/phase2-live-postrestart.json` (capturada `2026-09-13T16:44:33.894Z`, SHA-256 `45480039f54fb61adf96ceefb6ad1981dd3723871f1199c03f160f590eaf5cf2`): registró `stock_minimo_episodios=3`, `notificaciones_sistema=12`, las demás tablas A en cero y cambios de `updated_at` en 72 filas de `permisos_rol` y 54 de `permisos_ubicacion`, sin cambios en filas, flags o autores. Son efectos históricos del arranque, separados de la línea base transaccional, no una consulta actual. El resultado de aquella entrega fue **PURGE_COMMITTED_WITH_VALIDATION_LIMITATIONS**, por observaciones en suites y bloqueo de UI LIVE; no acredita el estado actual del proceso API.
...
- Un comando que se detiene en el primer fallo produce un resultado **parcial**, no el resultado total del proyecto. Registrar qué paquetes terminaron y cuáles no; completar los omitidos solo cuando el alcance permita ejecutarlos. Deduplicar los diagnósticos que se repiten al importar el mismo código entre proyectos. El primer fallo del chequeo principal ocultaba los dos errores de Alertas: la última evidencia completa registra **cuatro**, no dos.
...
- **Vencimiento: pendiente de lectura/presentación, no divergencia de datos.** El diagnóstico histórico descartó D3: documento y movimiento coinciden en **2026-10-15** en ambas notas. El corrimiento de un día entre pantalla e impreso queda por localizar en lectura/presentación; no se afirma que la base tenga dos fechas ni que se haya corregido el defecto.
