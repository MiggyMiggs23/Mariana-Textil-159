...
**Estado al 18 de septiembre de 2026: DDL operativo original aplicado y conservación verificada anteriormente; código E1 implementado. Las tres guardas independientes pasaron 84/84 controles y se instalaron SÓLO en el clon. El propietario autorizó su instalación operativa condicionada y después una ventana breve de arranque exclusivamente para identidad desde el pool real, con bloqueo de operaciones y parada antes del DDL. Esa ventana todavía NO se abrió: la preparación detectó que «27» corresponde a sentencias, mientras el inventario guardado de objetos E1 contiene 60. Se pidió aclarar la referencia, sin reinterpretar la condición ni ejecutar escrituras. E1 NO cerrado ni acreditado para operación normal. API PAUSADA. No arrancar ni reiniciar automáticamente la API, su workflow, inicializadores o mantenimiento.** Autorización ampliada: [autorizacion-ventana-e-instalacion.md](reports/e1-guardas-operativa-2026-09-18/autorizacion-ventana-e-instalacion.md); bloqueo actual y evidencia: [preflight-pendiente-inventario.md](reports/e1-guardas-operativa-2026-09-18/preflight-pendiente-inventario.md). Se conservan la autorización condicionada y el NO-GO anterior por falta de proceso en `reports/e1-guardas-temporales-2026-09-18/`. La ventana autorizada no permite operación normal ni reanudación posterior automática.
...
**Sesión solo cuando corresponde a dinero físico en efectivo.** Su soporte exige `sesionCajaId` explícita, abierta y del mismo sitio de origen, con `CAJA_FISICA`; nunca se fabrica ni se deduce una sesión. Una transferencia exige cuenta bancaria válida y ninguna sesión de caja. Una corrección o una operación de crédito sin dinero tampoco imputa sesión de caja. **La captura nueva de efectivo de crédito permanece DESHABILITADA en la aplicación**, incluidas devoluciones físicas en efectivo; tener esquema y validaciones no activa esa captura ni corrige el corte pendiente. **El DDL operativo aplicado anteriormente no incluyó los cierres temporales SQL** de efectivo, cobros retenidos y atribuciones. Su corrección se instaló y comprobó únicamente en el clon: tres funciones/triggers independientes, errores E1C01/E1P01/E1A01 y retiro individual sin tocar la integridad permanente. Existe autorización operativa condicionada, aún no ejecutada; no afirmar cierre SQL operativo sin cumplir sus condiciones y verificar la aplicación real. SQL, medición del clon y retiro en [resultado-y-propuesta.md](reports/e1-guardas-temporales-2026-09-18/resultado-y-propuesta.md).
...
**Históricos:** los **tres movimientos** conservados tras la migración siguen como **«Sin sitio determinado»**. La vía futura en `atribuciones_credito_e1` es append-only, solo para ADMIN/SUPERVISOR con permiso y sitio autorizado, evidencia y motivo obligatorios. Referencia el movimiento por ID **más su timestamp original exacto** y el snapshot de **cinco campos**: `cliente_id`, `tipo`, `importe`, `ticket_id`, `movimiento_origen_id`. La cadena de rectificación conserva esa identidad y enlaza mediante `anterior_id`, sin editar el original ni sobrescribir atribuciones previas. El propietario determina qué evidencia basta y cuál fue el sitio real. **La puerta de atribución está cerrada y no se ejecutó ninguna atribución.**
...
**Verificación ejecutada, sin declaración de API lista para operar:** typecheck completo en cero y frontend posterior en cero; 59 pruebas backend/contratos aprobadas; los 361 casos frontend ejecutados todos en una pasada global 360/361, seguidos por confirmación enfocada 2/2 tras corregir la prueba del único fallo; 66 pruebas nuevas vistas fallar con defectos reales en copias aisladas y pasar al restaurarlas. No hubo segunda pasada global frontend. Verificación offline en [validacion-codigo.md](reports/e1-ejecucion-2026-09-17/validacion-codigo.md). **Ensayo PostgreSQL posterior:** 20/20 POS, 59/59 clientes y 21/24 evidencia, con los siete productores ejercitados y tres INSERT de capturas cerradas admitidos por SQL y revertidos. Detalle en [resultado-ensayo-clon.md](reports/e1-ejecucion-2026-09-17/resultado-ensayo-clon.md). No se afirma suite completa del repositorio, prueba concurrente ni consumo/reverso de inventario. Los fixtures autorizados pertenecen exclusivamente al clon. Todas sus filas originales se conservaron; cero conexiones operativas durante el ensayo y API pausada.

**Corrección posterior de las tres brechas en el clon:** 84/84 controles, incluidas recepción y devolución física, compatibilidad SQL de los siete productores, retirada independiente y reinstalación exacta. Instalación conjunta hasta COMMIT confirmado: 29.141 ms; ejecución supervisada: 5.689 s, sin extrapolar esos tiempos a la operativa. Las 66 tablas, 55 funciones y 23 triggers permanentes conservaron sus filas/definiciones. Las tres guardas nuevas quedaron instaladas en el clon; la autorización operativa posterior continúa sin ejecutarse. La identidad actual se obtendrá únicamente durante la ventana autorizada, todavía pendiente de aclarar el inventario de referencia. **Estas guardas son temporales y removibles**, independientes de la integridad permanente: efectivo espera a **E2 y E3 coordinados**; pendientes, a **E3 y E5 con sus dependencias**; atribución histórica **no se libera automáticamente con ninguna entrega: requiere decisión explícita del propietario sobre la evidencia y el sitio**. Conservar el clon y el respaldo de Drive hasta cerrar E1. El reinicio del workspace terminó el proceso del clon, pero su directorio persistente se conserva; no confundirlo con una eliminación.
...
**CONTADOR** ve todo lo financiero y no toca la mercancía: consulta entradas, salidas y movimientos para cuadrar, pero no crea ni edita ninguno. Conserva sus permisos de pagos a proveedores y no administra el sistema. Los movimientos nuevos del ledger de crédito E1 exigen rol operativo autorizado: CONTADOR, SISTEMAS y BODEGA no los producen; E1 no crea un nuevo rol contable ni amplía esos permisos.
...
La matriz histórica de CONTADOR conserva `crear` en `cobros_pagos` y en `proveedores_finanzas`. Esas celdas no eluden el control de actor operativo del ledger E1. Convertir todo el perfil en consulta o rediseñar sus permisos requiere una decisión separada; E1 no modifica la matriz.
...
**Estado vigente, E1 — 2026-09-17: API PAUSADA, sin autorización de reanudación.** No arrancarla ni reiniciarla automáticamente al terminar código, documentación, pruebas o migración; tampoco iniciar workflows, inicializadores o mantenimiento. El arranque normal del 16 de septiembre, después de retirar `API_INSPECTION_BOOT`, es un antecedente histórico y no habilita otro arranque.

**Regla de autorización de arranque:** la API normal ejecuta escrituras de inicialización reproducibles e idempotentes; un arranque nunca equivale a solo lectura. Reanudarla requiere autorización expresa que contemple esas escrituras y el mantenimiento posterior. La autorización de un arranque pasado no es permiso permanente. En E1 solo se autorizó el DDL concreto de 27 sentencias ya aplicado: no se permiten escrituras adicionales, ni siquiera en el clon, y la prohibición incluye inicialización, fixtures y mantenimiento. No cambiar a modo de inspección ni invocar un reinicio normal para eludir esta pausa.
