...
**Precio mínimo — preparación inactiva autorizada el 2026-09-19; decisiones documentales del 2026-09-21:** el precio de lista no tiene techo para subir; el piso es el costo cuando el producto tenga costo registrado. A un producto sin costo registrado se le puede asignar precio de lista; esto no elimina el bloqueo de venta por costo del rollo. El bloqueo bajo costo está preparado en la mutación individual/masiva de Precios y su interfaz, con puertas cerradas; no cambia el bundle servido ni se presenta como vigente. La «confirmación bajo costo» anterior no es el comportamiento objetivo. El cambio de precios se configura como permiso de matriz para solo ADMIN, no como guard fijo por rol. Estas decisiones no acreditan implementación ni cobertura de otras rutas de edición/importación; esa verificación sigue pendiente antes de liberar. Alcance en «Decisiones futuras — remate, precios y purga».
...
- **Historial de precios sin movimientos — preparación inactiva autorizada (2026-09-19):** si el producto nunca tuvo movimientos, se borrará junto con sus renglones de `precio_historial`; si tuvo cualquier movimiento, no se borra. Se conserva la bitácora de cambios de precio (`CAMBIAR_PRECIO` en `routes/precios.ts`), no se borra su auditoría. La excepción está preparada únicamente para producto e historial, con puerta cerrada: el bloqueo servido no cambió. Se mantienen existencia cero en todos los sitios, credenciales ADMIN, auditoría y no reutilización del SKU. Las pruebas fueron con colaboradores en memoria, no contra PostgreSQL; no acreditan atomicidad ni concurrencia reales. Decisión original en `reports/prompt-u-respuestas-2026-09-18.md`; preparación y límites en `reports/tanda-nocturna-20260919/tarea-4/resultado.md`.
...
**Decisión E3 del propietario (2026-09-21), construcción apagada:** el cajero nunca ve una pregunta de naturaleza. Todo abono capturado desde Caja es dinero real recibido en ese momento y requiere caja abierta. Las recapturas de pagos recibidos antes, sin dinero nuevo, solo se capturan desde la pantalla del cliente, con motivo obligatorio; nunca entran a caja y **solo ADMIN puede capturarlas**. Se conserva la guarda E1: CONTADOR, SISTEMAS y BODEGA no pueden capturar recapturas, aunque tengan otro permiso de matriz. Se conserva el registro contable de crédito de la recaptura; no se simula una recepción física ni una sesión de caja.
...
**Límites de construcción E3:** código, interfaz y permisos nuevos permanecen apagados hasta autorización separada de liberación. El flujo dirigido sin ADMIN exige exactamente el saldo pendiente de las notas indicadas (P6) y no se habilita sin E5/E7; recibido pendiente no reduce deuda ni crea favor disponible. El esquema o retiro de guardas que se necesite se prepara con reversión, sin ejecución. Esta construcción no autoriza acceso a la base de la API, reinicios, cambios al bundle activo ni modificaciones del paquete o dist candidato de E2.

**Actores de recaptura E3 frente a E1 — resuelto por el propietario:** la recaptura queda exclusivamente en ADMIN. Se conserva la guarda E1 y, por tanto, CONTADOR, SISTEMAS y BODEGA no pueden capturar recapturas; un permiso de matriz no supera esa exclusión. SUPERVISOR, CAJA y TERMINAL tampoco quedan autorizados para recapturar por pertenecer a la lista general de actores de crédito. No se amplía ni se elude la guarda por esta construcción. Evidencia y límites en `reports/e3-20260921/backend.md`.
...
Fuente: decisiones del propietario conservadas textualmente en `reports/prompt-u-respuestas-2026-09-18.md` y autorización posterior de la tanda del 2026-09-19. **Construcción parcial e inactiva, con puertas API/UI cerradas.** Se prepararon código y SQL con reversión sin ejecutarlo. No se autoriza modificar bases, reiniciar la API, cambiar el bundle, abrir operación ni ejecutar la purga general. Esta autorización sustituyó solo el aplazamiento de construcción de remate, piso de precio y borrado individual de producto sin movimientos con su historial.

1. **Remate — preparación parcial inactiva; conflicto de matriz RESUELTO.** «Marcar remate» es un permiso configurable preparado que por defecto solo tiene ADMIN, sin techo fijo de roles. Hay handler, adaptador y componente sin montar, con puertas cerradas, por rollo y con motivo obligatorio; no se expone al frontend servido. La Tanda A del 2026-09-22 conectó en fuente la venta bajo costo con las marcas físicas, conservando `REMATE_RELEASED=false`: OFF no consulta el esquema preparado ni cambia el comportamiento previo. Para BOLSA, ON verifica dentro de la transacción los costos de las fuentes reales devueltas por FIFO, sin cambiar su algoritmo; una fuente bajo costo sin marca provoca rollback. ON registra la señal en auditoría y conserva ingresos y costos físicos en el ledger para reflejar la pérdida. El contrato/montaje de remate y su señal histórica visible en Ticket/Nota no se declaran implementados por esa evidencia de auditoría. No se declara remate completo ni liberable. La regla objetivo sigue siendo que solo esos rollos puedan venderse bajo costo, sin ampliar la marca a todo el producto ni permitir al cajero eludir permisos. **Retiro de marca resuelto documentalmente (2026-09-21):** ADMIN puede quitarla con motivo obligatorio; el retiro queda en el historial y no se borra la marca original. No acredita implementación, no redefine los permisos de otros roles ni autoriza editar la marca original; la edición distinta del retiro sigue pendiente.
2. **Precios — bloqueo preparado e inactivo junto con remate.** El precio de lista no tiene techo para subir. El piso es el costo cuando el producto tenga costo registrado; si no lo tiene, se permite asignar precio de lista y se conserva el bloqueo de venta por costo del rollo. El helper real de precio y su UI contienen preparación bajo puertas cerradas, sin alterar el cálculo canónico de costo. **Cambio de precios resuelto documentalmente (2026-09-21):** solo ADMIN mediante permiso configurado en la matriz, nunca como veto fijo por rol. La implementación de estas decisiones y la cobertura de otras rutas de edición/importación siguen por verificar antes de liberar; no se declaran completas por esta actualización.
