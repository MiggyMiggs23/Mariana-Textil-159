...
Estado de activación: **pendiente de autorización para reinicio normal de API**. No se reinició la API ni se crearon usuarios, sesiones o movimientos de prueba en esta ampliación. Un proceso anterior de la API ocupa 8080 y el workflow informa EADDRINUSE; activar el contrato requiere reemplazar ese proceso y permitir los inicializadores habituales, que pueden escribir. El frontend sí se reinició y la vista previa real mostró login/401. No se aprobó navegación autenticada ni se publicó una versión. Typecheck completo en cero, codegen estable y builds aprobados; los últimos textos se comprobaron además con seis pruebas enfocadas y typecheck frontend. Resultado por paquete, capturas y límites en `reports/prompt-g-after/entrega.md`.