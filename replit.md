...
**Decisión futura del propietario (2026-09-18), sin construir:** el precio de lista no tiene techo para subir; para bajar, el mínimo es el costo. Precios deberá bloquear precio de lista menor al costo en lugar de solo guardar `advertenciaBajoCosto` en `routes/precios.ts`. Se construye junto con la nueva entrega de remate. La «confirmación bajo costo» anterior se conserva como registro histórico, no como comportamiento objetivo; no se afirma que el bloqueo nuevo esté implementado. Alcance y conflictos pendientes en «Decisiones futuras — remate, precios y purga».
...
- **Historial de precios sin movimientos — RESUELTO para construcción futura (2026-09-18):** si el producto nunca tuvo movimientos, se borrará junto con sus renglones de `precio_historial`; si tuvo cualquier movimiento, no se borra. Se conserva la bitácora de cambios de precio (`CAMBIAR_PRECIO` en `routes/precios.ts`), no se borra su auditoría. Esta decisión sustituye la detención anterior por la relación obligatoria del historial y la prohibición de eliminar esos renglones en este caso específico. Se construye junto con remate y precios, no ahora; el bloqueo actual no se modificó. Se mantienen los demás requisitos de borrado, incluida existencia cero en todos los sitios, credenciales ADMIN, auditoría y no reutilización del SKU. La lectura estática confirma el registro `CAMBIAR_PRECIO`, no la cobertura de históricos en la base. Detalle en `reports/prompt-u-respuestas-2026-09-18.md`.
...
**Exactamente una de cuatro naturalezas, sin valor por defecto:**
...
El encabezado Cobrado también alimenta la banda «Cobranza del periodo» del tablero; véase «Tablero: venta y cobranza». No equivale a Contado cobrado de «POS y Caja — regla contable vigente». Véase también «Cobrado: dos definiciones sin reconciliar», cuya decisión corresponde al propietario. El orden visual aprobado es Cobranza, Matriz de Operaciones y Ventas; las secciones posteriores conservan su orden. Cobranza agrupa seis tarjetas iguales: primero Total en efectivo, Efectivo facturado y Efectivo sin factura; después Cuentas No Fiscales, Cuentas Fiscales y Por cobrar. Por cobrar conserva su naturaleza de cartera, subtítulo, borde punteado y detalle canónico; no se suma a Cobrado. Ventas contiene Ventas totales, Contado y Ventas a crédito, en tres tarjetas iguales a todo el ancho. El encabezado, filtros y exportaciones se mantienen arriba.
...
La pantalla contiene **Vendido**, **Por cobrar (notas de crédito al día)** y **Cobrado**. Cuentas por cobrar nunca se suma con las tres cuentas reales bajo una etiqueta de ingreso. Los porcentajes de Caja física, Cuenta fiscal y Cuenta no fiscal se calculan sobre Cobrado. También los de Efectivo facturado y Efectivo sin factura usan Cobrado como denominador; estos dos porcentajes de presentación están autorizados en frontend, sin alterar importes ni predicados.
...
**P9/P10 — conflicto RESUELTO para E11 (2026-09-18):** al construir E11, todas las cuentas CONTADOR pasarán a ContadorF y perderán todo acceso fuera de lo facturado, así como la capacidad de registrar pagos a proveedor o pagos de cliente. Sustituye «CONTADOR ve todo lo financiero» y la conservación de sus permisos de pagos; no se conservan permisos u overrides que contradigan ese alcance. No se migraron cuentas ni se cambiaron permisos ahora. ContadorA mantiene el alcance del plan y su asignación manual por el propietario.
...
**CONTADOR — transición futura a ContadorF:** su alcance objetivo en E11 es solo facturas, clientes facturados y ventas facturadas, con la aceptación de conciliación definida en P9. No verá lo financiero no facturado, Fondo ni pagos a proveedor, ni registrará pagos de cliente o proveedor. No toca mercancía ni administra el sistema. Hasta construir E11, la implementación permanece sin cambios; esta descripción no acredita una migración. Los movimientos nuevos del ledger de crédito E1 exigen rol operativo autorizado: CONTADOR, SISTEMAS y BODEGA no los producen; E1 no crea un nuevo rol contable ni amplía esos permisos.
...
- **Alta prioridad: asimetría de cobranza Cruces/global**, descrita con causa y evidencia histórica en «Cobrado: dos definiciones sin reconciliar».
...
## Cobrado: dos definiciones sin reconciliar

**Decisión abierta del propietario.** El código conserva dos mediciones de preguntas distintas que compartían el nombre Cobrado. El renombre del tablero no resuelve por sí solo la decisión documental ni propone una unificación:

- La frase histórica **Ventas = Cobrado + Ventas a crédito**, hoy expresada como **Ventas = Contado cobrado + Ventas a crédito**, pertenece a «POS y Caja — regla contable vigente». En `artifacts/api-server/src/lib/admin-analytics.ts`, `getSalesSummary` usa `collectedTicketPredicate` para contado y no suma los abonos del libro de crédito a esa tarjeta.
- **Cobrado = Contado + Abonos + Saldos a favor** pertenece a «Cuentas Destino». En el mismo archivo, `destinationReadModel` incluye las ramas del libro y `getDestinationAccounts` construye el encabezado; los reversos se netean en sus grupos. `useSharedCuentasDestino`, en `artifacts/mariana-textil/src/hooks/use-shared-cuentas-destino.ts`, alimenta tanto esa pantalla como la banda «Cobranza del periodo» del tablero.
...
- **Contado cobrado de Caja en Tiempo Real** consulta tickets/pagos de caja, no los ABONO del ledger. Su banda separada **Cobranza del periodo** comparte el lector de Cuentas Destino, que sí consulta ABONO con fecha efectiva y excluye los que tienen cuenta destino nula. Véase «Cobrado: dos definiciones sin reconciliar»; el renombre y la banda no corrigieron el tratamiento de históricos ni la asimetría por sitio.
...
- **Nomenclatura pendiente:** `TicketDetailPage` de `artifacts/mariana-textil/src/pages/ticket-detail.tsx` conserva el título `Ticket #${folio}` y «Cancelar Ticket» aun para una nota. PENDIENTE del encabezado documental se basa en cobro/autorización y no significa lo mismo que PENDIENTE del estado de nota derivado del ledger.
...
## Prompt P — criterio aprobado; ejecución detenida en exportaciones
...
**Grupo 1 no aprobado; grupos 2–4 no iniciados.** Las exportaciones parciales omiten abonos ordinarios sin ticket; requieren atribución de porciones desde la proyección global, no desde aplicaciones persistidas tomadas como sustituto del FIFO. Además, falta conservar el saldo pendiente de notas permitidas y retirar una clasificación global no autorizada del XLSX. Se detuvo la implementación por el aumento de alcance, sin reiniciar servicios ni crear usuarios, sesiones, migraciones o datos.
...
Diseño aprobado y construcción offline autorizada: `reports/e2-apertura-limitada/diseno-a-c-antes-de-implementar.md`. Hay una implementación candidata inactiva, pausada por el bloqueo prioritario del bundle: 40/40 pruebas focalizadas y typecheck API exit 0; revisión independiente, reconciliación SQL, negativos ampliados y comprobaciones globales pendientes. No está lista para abrir; no combinar el SQL refund anterior con el nuevo candidato. Estado preciso en `reports/e2-apertura-limitada/evidencia-a-c/estado-offline.md`.
...
**Prompt P permanece detenido por instrucción expresa del propietario: Grupo 1 sin cerrar ni aprobar; grupos 2, 3 y 4 sin continuar.** Las decisiones de E1 están recogidas en su sección canónica; las demás alternativas de Prompt Q no constituyen autorización para ampliar el trabajo.
...
- **P10 / E11 — sustituye la pregunta 10 y el pendiente de asignación del plan:** todas las cuentas CONTADOR existentes pasan a ContadorF; el propietario asigna ContadorA a mano. Al construir E11 pierden todo acceso fuera de lo facturado y no pueden registrar pagos a proveedor ni de cliente. Es una decisión para la futura migración, no una migración realizada.
- **P12 / E12 — conflicto de retiros RESUELTO:** pagos/retiros superiores al saldo registrado se bloquean; el desbloqueo ADMIN, con motivo obligatorio e historial, aplica **solo a caja**, no al Fondo. Sustituye la amplitud inicial que incluía Fondo en la excepción. Se conserva el límite de retiro de E10. El propietario dice además «El Fondo nunca queda en negativo»; su choque con la regla de inversos contables negativos queda reportado separadamente, sin resolver. Esta respuesta no modifica el límite de crédito del cliente ni habilita el desbloqueo ahora.
...
Las respuestas textuales y las resoluciones de P9/P10, retiros P12/E10, remate/matriz y producto/historial se conservan en el informe citado. Siguen pendientes la tensión de la purga completa y el nuevo conflicto de inversos negativos del Fondo. No se dan por contestadas otras preguntas del plan ni se altera el estado inactivo de los flujos preparados.

## Decisiones futuras — remate, precios y purga

Fuente: decisiones del propietario conservadas textualmente en `reports/prompt-u-respuestas-2026-09-18.md`. **Solo documentación: no construir remate ni precios, no preparar ni ejecutar purga, no modificar base ni reiniciar la API.**

1. **Remate — entrega nueva, no construir todavía; conflicto de matriz RESUELTO.** «Marcar remate» será un permiso configurable de la matriz que por defecto solo tiene ADMIN, no una restricción fija en código. Esto sustituye la formulación anterior de exclusividad inmutable de ADMIN. Se marcan rollos específicos con motivo obligatorio; solo esos rollos pueden venderse por debajo del costo. El cajero no puede eludir el permiso ni vender bajo costo rollos sin marcar. La venta queda señalada como remate y su pérdida aparece en los reportes de utilidad. Fuera de esta excepción futura, vender por debajo del costo sigue bloqueado. No se extiende la marca a todo un producto ni se declara habilitada.
2. **Precios — construir junto con remate.** El precio de lista no tiene techo para subir. Para bajar, el mínimo es el costo; Precios deberá bloquear un precio de lista menor al costo, en lugar de solo guardar la advertencia (`routes/precios.ts`, `advertenciaBajoCosto`). Se registra el cambio futuro respecto de la confirmación bajo costo descrita históricamente, sin modificar el cálculo de costo ni inventar una regla para costo desconocido.
3. **Purga pendiente — al terminar las entregas y cuando el propietario lo decida.** Se hará una purga completa de datos de prueba con el mismo procedimiento del 13 de septiembre: respaldo, ensayo en copia desechable y ejecución autorizada. Incluye expresamente los movimientos #51, #52 y #53 sin sitio. **No se prepara ni se ejecuta nada ahora:** no se reutilizan autorizaciones ni listas históricas como permiso para una nueva ejecución. Los tres movimientos continúan hoy «Sin sitio determinado»; su inclusión futura no los atribuye ni los elimina ahora.
...
El registro histórico de confirmación bajo costo no se reescribe como si ya hubiera sido un bloqueo. P9/P10 y el alcance del desbloqueo P12 están resueltos para construcción futura; el conflicto nuevo de inversos negativos se reporta en E10. Estas decisiones no dan por terminada ninguna entrega ni abren flujos operativos.

## E10 — Fondo de Mariana

**P12/E10 — conflicto de retiros RESUELTO (2026-09-18):** el desbloqueo ADMIN motivado e histórico por insuficiencia aplica solo a caja. El retiro del Fondo no puede exceder saldo; no tiene esa excepción. Sustituye la inclusión inicial de Fondo en el desbloqueo, sin implementar cambios ahora. **Nuevo conflicto SIN RESOLVER:** «El Fondo nunca queda en negativo» choca con la regla vigente de inversos contables que admite saldo negativo. No se decide por cuenta propia si bloquear esos inversos, acotar la nueva frase a egresos físicos o cambiar su procedimiento; la regla de inversos se conserva señalada como pendiente. Véase `reports/prompt-u-respuestas-2026-09-18.md`.
...
- No se edita ni elimina un movimiento. Se corrige mediante un inverso exacto,
  ...
  **Conflicto nuevo pendiente:** la admisión de saldo negativo por inverso
  choca con «El Fondo nunca queda en negativo». No se sustituye esa regla
  de inversos sin respuesta del propietario; el desbloqueo solo en caja
  ya está resuelto y no habilita retiros del Fondo sobre saldo.
