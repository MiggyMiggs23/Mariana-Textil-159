...
# Extract original Ventas section (up to the next section)
...
# Modify original Ventas:
# 1. extract the top cards grid
top_cards_match = re.search(r'(<div className="grid w-full min-w-0 grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">.*?</div>)', original_ventas, re.DOTALL)
top_cards_block = top_cards_match.group(1)

# 2. replace grid-cols-4 with grid-cols-3
top_cards_block = top_cards_block.replace('xl:grid-cols-4', 'xl:grid-cols-3')

# 3. remove Por cobrar card
...
# 4. create the new Ventas section and Matriz block separately
# But wait, original Matriz is also inside the same <section className="space-y-6"> for Ventas.
matriz_match = re.search(r'(<div>\s*<Card className="h-full">\s*<CardHeader>\s*<CardTitle className="text-lg flex items-center gap-2">\s*Matriz de Operaciones \(Ventas\).*?</div>\s*</Card>\s*</div>)', original_ventas, re.DOTALL)
...
# Extract the collection-detail-row
collection_row_match = re.search(r'(<div data-preview="collection-detail-row".*?</div>\s*</div>)', rest, re.DOTALL)
collection_row = collection_row_match.group(1)

# 1. extract Cobrado card
...
# Everything after collection_row
rest_after_collection = rest.split(collection_row)[1]
