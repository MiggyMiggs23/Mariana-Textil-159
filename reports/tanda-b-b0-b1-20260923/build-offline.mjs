...
write(path.join(report,"manifest.json"), { status: "BUILT_NOT_VALIDATED_MAIN_PENDING", revision,
  ...
  outputs, pending: ["REAL_READONLY_CAPTURE_EFFECTIVE_RUNTIME_PIN", "REAL_CURRENT_PREFLIGHT", "MINIMAL_FIXTURE_EXACT_DELTA_FOUR_REPLACEMENTS", "CANDIDATE_STARTUP_PRESERVATION", "MAIN_REVIEW"], sealed: false });
fs.writeFileSync(path.join(report,"release-assets.sha256"), Object.entries(outputs).map(([f,h]) => `${h}  ${f}\n`).join(""), { flag: "wx" });
fs.writeFileSync(path.join(report,"manifest.sha256"), `${digest(fs.readFileSync(path.join(report,"manifest.json")))}  reports/tanda-b-b0-b1-20260923/manifest.json\n`, { flag: "wx" });
