...
export function projectAuxiliary(live,before,after) {
  ...
  for(const section of ["riTriggers","dependencies"]) {
    const old=unique(before[section],canonical,"aux fixture before");
    const next=unique(after[section],canonical,"aux fixture after");
    const base=unique(live[section],canonical,"aux real B0");
    assert.equal(base.size,live[section].length,"Auxiliary identity collision in live capture");
    const removed=[...old].filter(([k])=>!next.has(k));
    const added=[...next].filter(([k])=>!old.has(k));
    for(const [k,r] of removed) {
      assert.equal(section,"dependencies","Existing FK RI semantics must remain identical");
      assert.ok(authorizedIdentities.has(r.source.identity),`Unauthorized dependency removal ${k}`);
      assert.ok(base.has(k),"Removed dependency absent from real B0");base.delete(k);
    }
    for(const [k,r] of added) {
      assert.ok(!base.has(k),`Auxiliary addition collides with B0: ${k}`);base.set(k,r);
    }
    projected[section]=[...base.values()].sort((a,b)=>cmp(canonical(a),canonical(b)));
    delta[section]={removed:removed.map(([,r])=>r),added:added.map(([,r])=>r)};
  }
