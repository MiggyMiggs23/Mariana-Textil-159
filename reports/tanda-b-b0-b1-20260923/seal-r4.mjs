...
verifyPreparation();
const dir=path.join(report,"r4"),files={};
...
for(const p of ["scripts/src/release-catalog-comparison.mjs","scripts/src/e5-sql-case-regression.test.mjs",
  ...
  "reports/tanda-b-b0-b1-20260923/prepare-r4.mjs","reports/tanda-b-b0-b1-20260923/seal-r4.mjs"])
  files[p]=digest(fs.readFileSync(path.join(root,p)));
write(path.join(dir,"preparation-inputs.json"),{status:"R4_PREPARATION_ONLY_MAIN_EXECUTION_PENDING",files,
  previousSealSha256:digest(fs.readFileSync(path.join(report,"r3/preparation-inputs.json")))});
