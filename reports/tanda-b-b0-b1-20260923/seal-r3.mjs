...
verifyPreparation();
const dir=path.join(report,"r3"),files={};
function walk(p){for(const e of fs.readdirSync(p,{withFileTypes:true})) {
  const f=path.join(p,e.name);
  if(e.isDirectory())walk(f);else files[path.relative(root,f)]=digest(fs.readFileSync(f));
}}
walk(dir);
for(const p of ["scripts/src/release-catalog-comparison.mjs","scripts/src/e5-sql-case-regression.test.mjs",
  ...
  "reports/tanda-b-b0-b1-20260923/prepare-r3.mjs","reports/tanda-b-b0-b1-20260923/seal-r3.mjs"])
  files[p]=digest(fs.readFileSync(path.join(root,p)));
write(path.join(dir,"preparation-inputs.json"),{status:"R3_PREPARATION_ONLY_MAIN_EXECUTION_PENDING",files,
  previousSealSha256:digest(fs.readFileSync(path.join(report,"r2/preparation-inputs.json")))});
