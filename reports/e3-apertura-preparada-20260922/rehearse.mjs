...
import { hash, readCatalog, verifyCatalog } from "./release-preflight.mjs";

const workspace = "/home/runner/workspace";
const packageRelative = "reports/e3-apertura-preparada-20260922";
...
fs.symlinkSync(path.join(workspace, "artifacts/api-server/dist-e3-apertura-20260922"),
  path.join(runRoot, "artifacts/api-server/dist-e3-apertura-20260922"));
fs.symlinkSync(path.join(workspace, "artifacts/mariana-textil/dist-e3-apertura-20260922"),
  path.join(runRoot, "artifacts/mariana-textil/dist-e3-apertura-20260922"));
...
const runtime = {
  ...
  E3_ENABLED: "true", E3_DIRECTED_ENABLED: "false", REMATE_RELEASED: "false", REMATE_UI_RELEASED: "false",
...
const results = {
  status: "FAIL", sourceRevision: JSON.parse(fs.readFileSync(path.join(packageDir,"preparation-status.json"))).sourceRevision,
...
WHERE e.enumtypid=t.oid AND n.nspname='public' AND t.typname=${literal(row.parent)} AND e.enumlabel=${literal(row.name)};`).join("\n"));
  ...
  const gate = commandResult(process.execPath, [path.join(packageDir, "release-preflight.mjs")],
    { env: { ...runtime, E3_DIRECTED_ENABLED: "true" } });
  fs.writeFileSync(path.join(evidence, "negative-gate.log"), gate.stdout + gate.stderr);
  assert.notEqual(gate.status, 0);
  assert.match(gate.stderr, /Closed feature environment mismatch: E3_DIRECTED_ENABLED/);
  ...
  fs.cpSync(path.join(workspace, "artifacts/api-server/dist-e3-apertura-20260922"),
    path.join(alteredRoot, "artifacts/api-server/dist-e3-apertura-20260922"), { recursive: true });
  fs.cpSync(path.join(workspace, "artifacts/mariana-textil/dist-e3-apertura-20260922"),
    path.join(alteredRoot, "artifacts/mariana-textil/dist-e3-apertura-20260922"), { recursive: true });
  fs.appendFileSync(path.join(alteredRoot, "artifacts/api-server/dist-e3-apertura-20260922/index.mjs"), "\n// isolated hash control\n");
  ...
  // Pino worker delivery is asynchronous. Health alone must not race the
  // mandatory INSPECTION proof; wait boundedly, keeping the candidate alive.
  let logged = "";
  ...
  assert.match(logged, /^E3_OPEN_RELEASE_PREFLIGHT=PASS \{/m);
  ...
  const bundle = path.join(workspace, "artifacts/api-server/dist-e3-apertura-20260922");
