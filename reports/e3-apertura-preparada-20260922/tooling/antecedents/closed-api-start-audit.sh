#!/usr/bin/env bash
# Candidate E3 CLOSED only. Invoke from workspace root. Not wired into a workflow.
package=reports/e3-paquete-liberacion-preparado-20260922
...
export E3_ENABLED=false E3_DIRECTED_ENABLED=false REMATE_RELEASED=false REMATE_UI_RELEASED=false
stage=preflight
...
if ! printf '%s\n' "$preflight_output" | grep -q '^E3_CLOSED_RELEASE_PREFLIGHT=PASS {'; then
...
record_attempt 0
trap - EXIT
exec node --enable-source-maps /home/runner/workspace/artifacts/api-server/dist-e3-20260922/index.mjs