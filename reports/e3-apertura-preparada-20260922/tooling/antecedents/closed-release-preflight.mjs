// External fail-closed E3 CLOSED preflight. Never imports the application.
...
export function connectionEnvironment(env) {
  ...
  for (const key of ["FONDO_E10_ENABLED", "CREDIT_REFUNDS_ENABLED", "CREDIT_CASH_INCOME_ENABLED",
    ...
    "E3_ENABLED", "E3_DIRECTED_ENABLED", "REMATE_RELEASED", "REMATE_UI_RELEASED"]) {
  ...
  return {
    ...
    PGCONNECT_TIMEOUT: "5", PGAPPNAME: "e3-closed-release-preflight",
...
export function readCatalog(env) {
  const result = spawnSync("psql", ["-X", "-q", "-A", "-t", "-v", "ON_ERROR_STOP=1"], {
    env: connectionEnvironment(env), encoding: "utf8",
    input: readFileSync(path.join(directory, "release-catalog.sql"), "utf8"),
...
export function verifyCatalog(actual, expected, phase = "after") {
  ...
  return { database: actual.database, databaseOid: actual.databaseOid, rows: actual.schemaRows.length,
    schemaSha256, attributesSha256, phase, mode: "INSPECTION",
    e3: false, e3Directed: false, remate: false, cashGateRetirement: false };
}
...
export function main() {
  ...
  console.log(`E3_CLOSED_RELEASE_PREFLIGHT=PASS ${JSON.stringify(result)}`);
}
if (process.argv[1] && pathToFileURL(realpathSync(process.argv[1])).href === import.meta.url) {
  try { main(); } catch (error) { console.error(`E3_RELEASE_PREFLIGHT=FAIL ${error.message}`); process.exitCode = 1; }
}