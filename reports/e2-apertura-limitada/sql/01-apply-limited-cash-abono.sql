-- PREPARED OFFLINE ONLY. DO NOT EXECUTE WITHOUT A NEW, EXPLICIT DB AUTHORIZATION.
-- psql variables required: expected_identity_sha256, expected_plan_sha256.
-- This transaction admits only ABONO/INGRESO_FISICO/EFECTIVO at the E1 cash gate.
...
\else
  \echo 'REFUSED: plan digest does not match planned CLOSED-mode target'
  \quit 3
\endif
...
guard_functions AS (
  SELECT p.proname,
   ...
   WHERE n.nspname = 'public'
     AND p.proname IN (
       'e1_guard_cash_capture_closed',
       'e1_guard_pending_receipts_closed',
...
function_checks AS (
  SELECT
    (SELECT pg_catalog.count(*) = 3 FROM guard_functions) AS count_ok,
    (SELECT pg_catalog.count(*) = 3
       FROM guard_functions
      WHERE prokind = 'f' AND provolatile = 'v' AND proparallel = 'u'
        ...
        AND result_type = 'trigger' AND public_execute) AS metadata_ok,
    (SELECT prosrc = E'\nBEGIN\n  IF NEW.forma_pago::text = ''EFECTIVO''\n     AND NEW.naturaleza::text IN (''INGRESO_FISICO'', ''DEVOLUCION_FISICA'') THEN\n    RAISE EXCEPTION USING\n      ERRCODE = ''E1C01'',\n      MESSAGE = ''E1: la captura física de efectivo de crédito está deshabilitada.'';\n  END IF;\n  RETURN NEW;\nEND;\n'
       FROM guard_functions WHERE proname = 'e1_guard_cash_capture_closed') AS cash_closed_exact,
    (SELECT prosrc = E'\nBEGIN\n  RAISE EXCEPTION USING\n    ERRCODE = ''E1P01'',\n    MESSAGE = ''E1: el cobro retenido de crédito está deshabilitado.'';\n  RETURN NULL;\nEND;\n'
       FROM guard_functions WHERE proname = 'e1_guard_pending_receipts_closed') AS pending_exact,
    (SELECT prosrc = E'\nBEGIN\n  RAISE EXCEPTION USING\n    ERRCODE = ''E1A01'',\n    MESSAGE = ''E1: la atribución histórica de crédito está deshabilitada.'';\n  RETURN NULL;\nEND;\n'
       FROM guard_functions WHERE proname = 'e1_guard_historical_attribution_closed') AS attribution_exact
),
