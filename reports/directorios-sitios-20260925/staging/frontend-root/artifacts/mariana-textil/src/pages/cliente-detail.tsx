...
import { E7ClientExport } from "@/components/e7-readers";
import { e7ClientFinancialOn } from "@/lib/e7-feature-flags";
...
import { createAdjustment, downloadClientFile, getClientAnalytics, getPortfolio, getPurchases, getStats, saleTypeLabel, updateCreditTerms } from "@/lib/clientes-api";
...
export default function ClienteDetail() {
  ...
  const canViewSaleExits = hasPermission(user, Modules.SALIDAS_VENTA, "ver");
  ...
  const account = useGetClienteEstadoCuenta(id, undefined, {
    query: {
      enabled: canFinances && !e7ClientFinancialOn() && Number.isFinite(id),
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {canFinances && !e7ClientFinancialOn() && <><Button variant="outline" onClick={() => downloadClientFile(`/clientes/${id}/estado-cuenta.pdf`, `estado-cuenta-${id}.pdf`)} data-testid="button-export-account"><Download className="mr-2 h-4 w-4" />Descargar PDF</Button><Button variant="outline" onClick={() => window.print()} data-testid="button-print-account"><Printer className="mr-2 h-4 w-4" />Imprimir</Button></>}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="h-auto w-full justify-start overflow-x-auto">
            ...
            {canFinances && <TabsTrigger value="compras">Últimas ventas</TabsTrigger>}
...
function ClientAnalyticsBlocks({ query }: { query: { data?: Awaited<ReturnType<typeof getClientAnalytics>>; isLoading: boolean; isError: boolean; error: unknown } }) {
  return <div className="mt-4 space-y-4"><QueryState query={query}><div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle>Actividad del periodo</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-4"><Datum label="Última compra" value={query.data?.actividad.ultimaCompra ? date(query.data.actividad.ultimaCompra) : "Sin compras"} /><Datum label="Tickets" value={formatNumber(query.data?.actividad.tickets, { kind: "count" })} /><Datum label="Ticket promedio" value={formatNumber(query.data?.actividad.ticketPromedio, { kind: "money" })} /><Datum label="Ticket máximo" value={formatNumber(query.data?.actividad.ticketMaximo, { kind: "money" })} /><p className="col-span-2 text-sm text-muted-foreground">Resume tickets y montos monetarios del cliente en el periodo elegido.</p></CardContent></Card><Card><CardHeader><CardTitle>Mezcla de pagos</CardTitle></CardHeader><CardContent><ResponsiveTable headers={["Forma", "Importe", "Movimientos"]} rows={(query.data?.mezclaPagos ?? []).map((item) => ({ id: `payment-method-${item.forma ?? "SIN_FORMA"}`, cells: [item.forma ?? "Sin forma", formatNumber(item.importe, { kind: "money" }), formatNumber(item.movimientos, { kind: "count" })] }))} empty="Sin pagos en el periodo." /><p className="mt-3 text-sm text-muted-foreground">Suma importes monetarios y movimientos por forma de pago del cliente en el periodo elegido.</p></CardContent></Card></div><Card><CardHeader><CardTitle>Tendencia mensual</CardTitle></CardHeader><CardContent><ResponsiveTable headers={["Mes", "Tickets", "Ventas"]} rows={(query.data?.tendencia ?? []).map((item) => ({ id: `month-${item.mes}`, cells: [item.mes, formatNumber(item.tickets, { kind: "count" }), formatNumber(item.ventas, { kind: "money" })] }))} empty="Sin tendencia disponible." /><p className="mt-3 text-sm text-muted-foreground">Agrupa tickets y ventas monetarias del cliente por mes dentro del periodo elegido.</p></CardContent></Card><Card><CardHeader><CardTitle>Productos por unidad, tela y color</CardTitle></CardHeader><CardContent><ResponsiveTable headers={["SKU", "Tela", "Color", "Unidad", "Cantidad", "Ventas", "Utilidad"]} rows={(query.data?.productos ?? []).map((item) => ({ id: item.id, cells: [item.sku, item.tela, item.color, formatUnit(item.unidad), formatNumber(item.cantidad, { kind: "quantity" }), formatNumber(item.ventas, { kind: "money" }), formatNumber(item.margen, { kind: "money" })] }))} empty="Sin productos en el periodo." /><p className="mt-3 text-sm text-muted-foreground">Agrupa cantidad por unidad, ventas y utilidad monetaria del cliente por producto en el periodo elegido; la utilidad requiere costo.</p></CardContent></Card></QueryState></div>;
}
