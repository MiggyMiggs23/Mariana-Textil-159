...
export function ClientDirectory({ enabled, canFinances, isAdmin, authPartition }: {
  ...
  return <div className="space-y-4">
    <Card><CardContent className="space-y-4 p-4">
      ...
      {canFinances && <p className="text-xs text-muted-foreground">Movimientos ({periods[data?.appliedPeriod ?? period]}): ventas, notas y abonos del periodo elegido. Última actividad: fecha del último movimiento, sin limitar al periodo.</p>}
    </CardContent></Card>
            ...
            {canFinances && <>
              <TableCell className="whitespace-nowrap">{client.saldoActual == null ? <span className="text-muted-foreground">No disponible</span> : <>
                <div className="flex items-center gap-2">
                  <span className="font-semibold tabular-nums">{money.format(Number(client.saldoActual))}</span>
              ...
              <TableCell className="tabular-nums font-semibold">{client.movementCount ?? "—"}</TableCell>
