...
import {
  ...
  ListProveedoresDirectorioQueryParams,
  ListProveedoresDirectorioResponse,
...
// Additive directory read: operational GET /proveedores and its consumers remain unchanged.
// An entrada is one purchase regardless of the number of products/rolls it contains.
router.get(
  ...
  async (req, res, next): Promise<void> => {
    try {
      const parsed = ListProveedoresDirectorioQueryParams.safeParse(req.query);
      if (!parsed.success) {
        res.status(400).json({ error: "Periodo del directorio inválido." });
        return;
      }
      const { ubicacionId, scopeError } = resolveReadScope(req.auth!);
      if (scopeError) {
      ...
      const interval = period === "1m" ? sql`INTERVAL '1 month'`
        : period === "3m" ? sql`INTERVAL '3 months'`
        : sql`INTERVAL '1 year'`;
      const counts = await db.execute<{ proveedor_id: number; purchase_count: number }>(sql`
        SELECT e.proveedor_id, COUNT(*)::int AS purchase_count
        FROM entradas e
        WHERE e.proveedor_id IS NOT NULL
          AND (${ubicacionId ?? null}::int IS NULL OR e.ubicacion_id = ${ubicacionId ?? null})
          AND (${period === "all"}::boolean OR e.fecha >=
            ((CURRENT_TIMESTAMP AT TIME ZONE 'America/Mexico_City' - ${interval})
              AT TIME ZONE 'America/Mexico_City'))
        GROUP BY e.proveedor_id
      `);
      res.setHeader("Cache-Control", "private, no-store");
      res.json(ListProveedoresDirectorioResponse.parse({
        period,
