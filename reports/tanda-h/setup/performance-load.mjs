...
const out=root+"/reports/tanda-h/performance";
fs.mkdirSync(out,{recursive:true});
const cfg=JSON.parse(fs.readFileSync(privateRoot+"/worker-databases.json","utf8")).performance;
const fixture=JSON.parse(fs.readFileSync(root+"/reports/tanda-h/setup/fixture-manifest-redacted.json","utf8"));
...
if(identity.db!=="tanda_h_performance"||identity.port!==55444||identity.actor!=="h_performance")throw Error("Isolation failure");
...
try{
 ...
 md5('TANDA-H-PERF-'||g)::uuid,'2025-09-24'::timestamptz+((g-1)/150)*interval '1 day',
 ...
 SELECT 'H-PERF-'||g,($1::int[])[1+(g-1)%4],($2::int[])[1+(g-1)%3],$3,
 ...
 SELECT t.id,r.id,r.producto_id,1,100,100,100,50,50,'NORMAL' FROM tickets t JOIN rollos r ON r.serie='H-PERF-'||(t.folio-1800000000) WHERE t.folio>1800000000`);
