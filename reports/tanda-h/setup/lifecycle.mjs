...
const workers=["inventory","performance","permissions"], port=55444;
...
if(action==="start")start();
 ...
 for(const name of ["template",...workers,"witness"]){
  ...
  if(workers.includes(name)){
   const role="h_"+name,password=randomBytes(32).toString("hex");
   ...
   credentials[name]={url:`postgresql://${role}:${password}@127.0.0.1:${port}/${db}`,db,role,apiPort:43871+workers.indexOf(name),uiPort:43881+workers.indexOf(name)};
  }
  identities.push(identity);await c.end();
 }
 const connectMatrix=(await admin.query("select r.rolname,d.datname,has_database_privilege(r.rolname,d.datname,'CONNECT') allowed from pg_roles r cross join pg_database d where r.rolname like 'h_%' and d.datname like 'tanda_h_%' order by 1,2")).rows;
 ...
 const schema=command("pg_dump",["-h","127.0.0.1","-p",String(port),"-U","postgres","-d","tanda_h_template","--schema-only","--no-owner","--no-acl"]);
 ...
 if(action==="api"){
  ...
  Object.assign(env,{NODE_ENV:"test",PORT:String(cfg.apiPort),REQUIRE_ISOLATED_TEST_DATABASE:"1",TEST_DATABASE_URL:cfg.url,DATABASE_URL:"postgresql://postgres@127.0.0.1:55444/tanda_h_witness",APPLICATION_DATABASE_URL:"postgresql://postgres@127.0.0.1:55444/tanda_h_witness",SESSION_SECRET:fs.readFileSync(r+"/session-secret","utf8"),WORKER:worker});
 }else{
