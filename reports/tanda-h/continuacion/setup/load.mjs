...
const root=process.cwd(), privateRoot=root+"/private.local/tanda-h-resume";
const out=root+"/reports/tanda-h/continuacion/setup";
const cfg=JSON.parse(fs.readFileSync(privateRoot+"/worker-databases.json","utf8")).browser;
const fixture=JSON.parse(fs.readFileSync(root+"/reports/tanda-h/continuacion/setup/fixture-manifest-redacted.json","utf8"));
...
if(identity.db!=="tanda_hr_browser"||identity.port!==55445||identity.actor!=="hr_browser")throw Error("Isolation failure");
...
try{
 ...
 SELECT 'GA-PERF-'||g,($1::int[])[1+(g-1)%4],($2::int[])[1+(g-1)%3],$3,
