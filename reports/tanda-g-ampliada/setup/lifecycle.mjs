...
const root="/home/runner/workspace", r=root+"/.local/tanda-g-ampliada", report=root+"/reports/tanda-g-ampliada/setup";
const PG="/nix/store/bgwr5i8jf8jpg75rr53rz3fqv5k8yrwp-postgresql-16.10/bin";
const workers=["permissions","inventory","month","performance","failure"], port=55442;
...
if(action==="start")start();
 ...
 for(const name of ["template",...workers,"witness"]){
  ...
  if(name!=="witness"){
   identity.metrage=(await c.query("select count(*)::int n from productos where se_vende_por_metro")).rows[0].n;
   if(identity.metrage!==0)throw Error("Metrage gate not zero");
  ...
  if(workers.includes(name)){
   const role="ga_"+name,password=randomBytes(32).toString("hex");
   ...
   credentials[name]={url:`postgresql://${role}:${password}@127.0.0.1:${port}/${db}`,db,role,apiPort:43851+workers.indexOf(name),uiPort:43861+workers.indexOf(name)};
  }
  identities.push(identity);await c.end();
 }
 const connectMatrix=(await admin.query("select r.rolname,d.datname,has_database_privilege(r.rolname,d.datname,'CONNECT') allowed from pg_roles r cross join pg_database d where r.rolname like 'ga_%' and d.datname like 'tanda_ga_%' order by 1,2")).rows;
 ...
 const schema=command("pg_dump",["-h","127.0.0.1","-p",String(port),"-U","postgres","-d","tanda_ga_template","--schema-only","--no-owner","--no-acl"]);
 ...
 console.log("All five worker identities sealed; sibling CONNECT denied; metrage zero.");
 }finally{stop();}
 ...
 if(action==="api"){
  ...
  Object.assign(env,{NODE_ENV:"test",PORT:String(cfg.apiPort),REQUIRE_ISOLATED_TEST_DATABASE:"1",TEST_DATABASE_URL:cfg.url,DATABASE_URL:"postgresql://postgres@127.0.0.1:55442/tanda_ga_witness",APPLICATION_DATABASE_URL:"postgresql://postgres@127.0.0.1:55442/tanda_ga_witness",SESSION_SECRET:fs.readFileSync(r+"/session-secret","utf8"),WORKER:worker});
 }else{
...
}else throw Error("Usage: lifecycle.mjs start|seal|api WORKER|ui WORKER|teardown [--remove]");