...
import { crearRollo, transferirRolloInmediato, venderRollo, salidaMostrador, crearSalidaExtraordinaria, revertirMovimiento, consumirBolsasFifo, reconstruirCacheExistencias } from "@inventory-engine";
...
for (const site of f.sites.slice(0, 2)) for (const unit of ["METRO", "KILO", "PIEZA", "BOLSA"]) {
  ...
  for (const scenario of ["transfer-roundtrip", "sale-reverse", "mostrador-terminal", "extraordinary-terminal", ...(unit === "BOLSA" ? ["FIFO-valid", "FIFO-negative", "FIFO-insufficient"] : [])]) {
    ...
    try {
      await db.transaction(async (tx: any) => {
        ...
        if (scenario === "transfer-roundtrip") {
...
fs.writeFileSync("reports/tanda-g-ampliada/tarea-2/chains-check.json", JSON.stringify({ identity, constraint, records }, null, 2));
await pool.end();
