...
function item(role: string, estado = "PENDIENTE", tienda = 1, userTienda = 1) {
  const salida = wire(ListarSalidasDineroCajaResponse.parse({ salidas: [{
    ...
    cuentaOrigen: "CAJA_FISICA", createdAt: "2026-09-22T12:00:00Z",
...
test("E4-PROVIDER-REQUIRED", () => {
  panel(); select(/Tipo de Salida/i, "Proveedor"); fill(); submit();
  assert.equal(captureCalls.length, 0, "E4-PROVIDER-REQUIRED");
  select(/^Proveedor$/i, "Proveedor activo fixture"); submit();
  assert.equal(captureCalls.length, 1);
...
test("E4-GATE-OFF", () => {
  ...
  assert.ok(screen.queryByRole("button", { name: /^Registrar salida$/i }), "E4-GATE-OFF");
  assert.equal(Boolean(screen.queryByRole("combobox", { name: /Tipo de Salida/i })), false);
  assert.ok(screen.queryByText("Solo pagos operativos de Tienda Mariana."));
  assert.equal(visits.includes("useRevisarSalidaDineroCaja"), false);
});
test("E4-CAPTURE-DOUBLE", () => {
  panel(); fill(); act(() => { submit(); submit(); });
  assert.equal(captureCalls.length, 1, "E4-CAPTURE-DOUBLE");
});
test("E4-CAPTURE-CONTENT", () => {
  panel(); fill(); submit(); const first = captureCalls[0][0].data.claveOperacion;
  fail(captureCalls); fill("Otro gasto"); submit();
...
test("E4-PARENT-OFF-COCO", () => {
  ...
  assert.equal(Boolean(screen.queryByText("Salidas de dinero")), false, "E4-PARENT-OFF-COCO");
});
test("E4-PARENT-OFF-MARIANA", () => {
  setGate(false); user("CAJA", true, true, true); mount(<CobrosPage />);
  assert.ok(screen.queryByRole("button", { name: /^Registrar salida$/i }), "E4-PARENT-OFF-MARIANA");
  assert.equal(Boolean(screen.queryByRole("combobox", { name: /Tipo de Salida/i })), false);
});