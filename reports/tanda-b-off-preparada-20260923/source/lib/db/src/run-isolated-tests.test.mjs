...
test("native reporter parser accepts real terminals and rejects false names", () => {
  const realLog = readFileSync(join(
    process.cwd(),
    "reports/actor-suites/runs/2026-09-23T05-50-23.441Z-0fb10cb2/03-stdout.log",
  ), "utf8");
  const suitePath =
    "artifacts/api-server/src/aplicaciones-pago-proveedor.integration.test.ts";
  const parsed = parseNativeTestEvidence(realLog, suitePath);
  assert.equal(parsed.tests, 2);
  assert.equal(parsed.pass, 2);
  assert.throws(
    () => parseNativeTestEvidence(
      realLog.replace(/^✔ .+ \(/m, "✔ terminal inventado no declarado ("),
      suitePath,
    ),
    /no corresponden a nombres declarados/,
  );
});

test("custom file-only terminal never qualifies as native case evidence", () => {
  const customHarnessLog = readFileSync(join(
    process.cwd(),
    "reports/actor-suites/runs/2026-09-23T05-50-33.496Z-6b2c4dea/05-stdout.log",
  ), "utf8");
  assert.throws(
    () => parseNativeTestEvidence(
      customHarnessLog,
    ...
    /exactamente 49 terminales node:test/,
  );
});
...
test("policy allows only prepare-smoke and the two explicitly reviewed suites", () => {
  const policy = getIsolatedTestPolicy();
  assert.equal(policy.default, "prepare-smoke");
  assert.deepEqual(policy.allowedSuites.map((suite) => suite.id).sort(), [
