...
import { useCrearEnviarSalidaVentaCliente, useGetUbicacionesSalida, useGetCurrentUser, Role } from "@workspace/api-client-react";
...
export function SalidaVentaClienteNueva() {
  ...
  const addSerie = () => {
    const value = serie.trim().toUpperCase();
    if (!value) return;
  ...
  const submit = () => {
    ...
    create.mutate({ data: { uuidCliente: crypto.randomUUID(), origenId, clienteId, series, nota: nota.trim() || null } }, {
      onSuccess: (salida) => setLocation(`/salidas/${salida.id}`),
  ...
  return <AppLayout><div className="mx-auto max-w-3xl space-y-5 pb-12">
    ...
    <Card><CardHeader><CardTitle>Datos de la salida</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      <div><label className="text-sm font-medium">Serie exacta del rollo</label><CampoEscaneo ref={scanRef} value={serie} onChange={setSerie} onScan={() => addSerie()} interpretRollCode={false} placeholder="Escanea la serie identificada y presiona Enter" /><p className="mt-1 text-xs text-muted-foreground">No se puede reservar por cantidad ni agregar líneas sin serie.</p></div>
