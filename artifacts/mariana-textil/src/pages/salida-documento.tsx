...
export default function SalidaDocumento() {
  ...
  if (tipo !== "salida" && tipo !== "recepcion") {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 text-center">
        <AlertCircle className="w-12 h-12 text-destructive mb-4" />
        <p className="text-lg font-medium text-slate-900">Tipo de documento inválido</p>
  ...
  const title = isRecepcion ? "RECEPCIÓN" : "SALIDA";

  const isSalidaAvailable = ["ENVIADA", "RECIBIDA", "CERRADA"].includes(salida.estado);
  const isRecepcionAvailable = ["RECIBIDA", "CERRADA"].includes(salida.estado);

  if ((isRecepcion && !isRecepcionAvailable) || (!isRecepcion && !isSalidaAvailable)) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 text-center">
  ...
  const dateStr = isRecepcion ? salida.fechaRecepcion : salida.fechaEnvio;
  const dateObj = dateStr ? new Date(dateStr) : null;
  const userRoleStr = isRecepcion ? "Recibido por" : "Enviado por";
  const userName = isRecepcion ? salida.nombreRecibidoPor : salida.nombreEnviadoPor;
  const note = isRecepcion ? salida.notaRecepcion : salida.notaEnvio;

  // Pagination logic: max 10 rolls per page
    ...
    <div className="min-h-[100dvh] bg-muted/20 flex flex-col">
      <div className="no-print p-4 border-b bg-background sticky top-0 z-10 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-bold">Vista previa de impresión ({title})</h1>
        </div>
          ...
          >
            ...
            <div className="flex justify-between items-center p-6 border-b">
              <div className="flex items-center gap-4">
                ...
                <h1 className="text-5xl font-black text-[#1e3a8a] tracking-tighter uppercase">{title}</h1>
            ...
            <div className="px-8 py-4">
              <div className="grid grid-cols-2 gap-x-12 gap-y-4">
                <div className="flex items-center border-b border-gray-300 pb-1">
                  ...
                  <span className="font-bold w-32 text-xs uppercase text-gray-500 tracking-wider">{userRoleStr}</span>
                  <span className="font-medium text-sm text-black">{userName || "N/A"}</span>
                </div>
                <div className="flex items-center border-b border-gray-300 pb-1">
              ...
              <table className="w-full text-left border-collapse border border-gray-200">
                <thead>
                  <tr className="bg-[#1e3a8a] text-white">
                    ...
                    <th className="py-2 px-3 text-xs font-bold uppercase tracking-wider">Serie Rollo</th>
                    <th className="py-2 px-3 text-xs font-bold uppercase tracking-wider">Producto</th>
                    <th className="py-2 px-3 text-xs font-bold uppercase tracking-wider text-right w-24">Enviado</th>
                    {isRecepcion && (
                      <>
                        <th className="py-2 px-3 text-xs font-bold uppercase tracking-wider text-right w-24">Recibido</th>
                        <th className="py-2 px-3 text-xs font-bold uppercase tracking-wider text-center w-48">Diferencia</th>
                      </>
                    )}
                  </tr>
                </thead>
                <tbody>
                  {pageRollos.map((rollo, index) => {
                    ...
                    const hasDiff = rollo.recibido === false || Number(rollo.diferencia) > 0;
                    return (
                      <tr key={index} className="border-b border-gray-200 even:bg-gray-50">
                        ...
                        <td className="py-1 px-3 text-sm text-gray-800 truncate max-w-[250px]">{line?.skuProducto} - {line?.colorProducto}</td>
                        ...
                        {isRecepcion && (
                          <>
                            <td className="py-1 px-3 text-right text-sm font-medium">
                              {rollo.recibido === false ? "No" : (rollo.cantidadRecibida || "-")}
                            </td>
                            <td className="py-1 px-3 text-center text-xs truncate max-w-48">
                              {hasDiff ? (
                                <span className="font-bold text-red-600">
                                  {rollo.diferencia && Number(rollo.diferencia) !== 0 ? `${rollo.diferencia} ` : 'SÍ '}
                                  {rollo.notaDiferencia ? `(${rollo.notaDiferencia})` : ''}
                                </span>
                ...
                <div className="flex justify-between items-end gap-8">
                  ...
                  <div className="w-[30%] bg-gray-50 border border-gray-200 rounded-md p-3 h-24">
                    ...
                    <div className="text-xs text-black italic leading-tight">
                      {note || salida.notaSolicitud || "Sin observaciones."}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-2 gap-8">
                    ...
                    <div className="text-center">
                      <div className="border-b border-black w-full mb-1 h-12"></div>
                      <div className="text-[10px] font-bold uppercase text-gray-700 tracking-wider">
                        {isRecepcion ? "Recibió (Almacén Destino)" : "Despachó (Almacén Origen)"}
                      </div>
                    </div>
                  </div>
                </div>
