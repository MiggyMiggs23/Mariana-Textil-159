...
const money = (value: string | number | undefined | null) => Number(value ?? 0).toLocaleString("es-MX", { style: "currency", currency: "MXN" });
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <Tabs defaultValue="datos">
          ...
          {canCredit && <TabsContent value="credito"><QueryState query={credit}>{(() => {
            ...
            return <><div className="mb-4 flex justify-end">{canEditCredit && !client.esSistema && <Button variant="outline" onClick={() => { setCreditLimit(credit.data?.limiteCredito ?? ""); setCreditDays(String(credit.data?.diasCredito ?? client.diasCredito)); setCreditOpen(true); }}>Editar términos</Button>}</div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Kpi label="Límite" value={money(credit.data?.limiteCredito)} /><Kpi label="Saldo actual" value={money(credit.data?.saldoActual)} /><Kpi label="Disponible" value={money(credit.data?.creditoDisponible)} /><Kpi label="Días de crédito" value={String(credit.data?.diasCredito ?? client.diasCredito)} /><Kpi label="Total vencido" value={money(credit.data?.totalVencido)} />{credit.data?.primeraCompra && <Kpi label="Primera compra" value={date(credit.data.primeraCompra)} />}{credit.data?.ultimaActividad && <Kpi label="Última actividad" value={date(credit.data.ultimaActividad)} />}</div>{utilization !== null && Number.isFinite(utilization) && <Card className="mt-4"><CardContent className="pt-6"><div className="flex justify-between text-sm"><span>Utilización</span><strong>{utilization.toFixed(1)}%</strong></div><div className="mt-2 h-3 overflow-hidden rounded-full bg-muted"><div className={`h-full ${utilization > 100 ? "bg-destructive" : utilization > 80 ? "bg-amber-500" : "bg-emerald-500"}`} style={{ width: `${Math.min(100, utilization)}%` }} /></div></CardContent></Card>}<Card className="mt-4"><CardHeader><CardTitle>Antigüedad real</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-5">{apiAging.length ? apiAging.map((bucket, index) => <Aging key={String(bucket.rango ?? index)} label={String(bucket.rango ?? bucket.bucket ?? "Periodo")} value={String(bucket.importe ?? bucket.saldo ?? "")} />) : aging ? <><Aging label="Por vencer" value={aging.porVencer} /><Aging label="1–30 días" value={aging["1_30"]} /><Aging label="31–60 días" value={aging["31_60"]} /><Aging label="61–90 días" value={aging["61_90"]} /><Aging label="+90 días" value={aging.mas90} /></> : <p className="col-span-full text-muted-foreground">Sin saldo pendiente.</p>}</CardContent></Card></>;
          })()}</QueryState></TabsContent>}
          {canFinances && <TabsContent value="estado"><QueryState query={account}><div className="mb-4 flex flex-wrap gap-2"><Select value={movementType} onValueChange={setMovementType}><SelectTrigger className="w-48"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todos los movimientos</SelectItem><SelectItem value="VENTA_CREDITO">Ventas a crédito</SelectItem><SelectItem value="ABONO">Pagos</SelectItem><SelectItem value="AJUSTE">Ajustes</SelectItem></SelectContent></Select><Input type="date" aria-label="Movimientos desde" value={movementFrom} onChange={(e) => setMovementFrom(e.target.value)} className="w-40" /><Input type="date" aria-label="Movimientos hasta" value={movementTo} onChange={(e) => setMovementTo(e.target.value)} className="w-40" /><Button variant="outline" onClick={() => downloadClientFile(`/clientes/${id}/estado-cuenta.xlsx`, `estado-cuenta-${id}.xlsx`)}>Excel</Button>{canCreatePayment && <Button onClick={() => setPaymentOpen(true)} data-testid="button-register-payment">Registrar pago</Button>}{canAdjust && <Button variant="outline" onClick={() => setAdjustmentOpen(true)} data-testid="button-register-adjustment">Ajuste</Button>}</div><Card><CardHeader><CardTitle>Movimientos, pagos y ajustes</CardTitle></CardHeader><CardContent><ResponsiveTable headers={["Fecha", "Tipo", "Folio", "Pago", "Referencia / notas", "Usuario", "Importe", "Saldo"]} rows={(account.data?.movimientos ?? []).filter((item) => (movementType === "all" || item.tipo === movementType) && (!movementFrom || (item.fechaEfectiva ?? item.fecha ?? "") >= movementFrom) && (!movementTo || (item.fechaEfectiva ?? item.fecha ?? "").slice(0, 10) <= movementTo)).map((item, index) => [date(item.fechaEfectiva ?? item.fecha), item.tipo ?? "Movimiento", item.ticketFolio ?? "—", item.formaPago ?? "—", [item.referencia, item.notas].filter(Boolean).join(" · ") || "—", item.nombreUsuario ?? "—", money(item.importe), money(item.saldoCorrido), String(index)])} empty="No hay movimientos en la cuenta." /></CardContent></Card><div className="mt-4"><Kpi label="Saldo actual" value={money(account.data?.saldoActual)} /></div></QueryState></TabsContent>}
          {canFinances && <TabsContent value="compras" className="space-y-4"><Period value={period} onChange={setPeriod} /><QueryState query={purchases}><Card><CardContent className="pt-6"><ResponsiveTable headers={["Fecha", "Folio", "Subtotal", "IVA", "Total", "Metros", "Kilos", "Margen"]} rows={filteredPurchases.map((item, index) => [date(item.fecha), item.folio ?? `#${item.id ?? "—"}`, money(item.subtotal), money(item.iva), money(item.total), item.metros ?? "0", item.kilos ?? "0", item.lineasSinCosto ? "N/D (sin costo)" : money(item.margen), String(item.id ?? index)])} empty="No hay compras en este periodo." /></CardContent></Card><p className="text-right text-sm font-semibold">Total del periodo: {money(filteredPurchases.reduce((sum, item) => sum + Number(item.total ?? 0), 0))}</p></QueryState></TabsContent>}
          {(canFinances || canPrices) && <TabsContent value="analitica" className="space-y-4"><Period value={period} onChange={setPeriod} />{canFinances && <QueryState query={stats}><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Kpi label="Compra acumulada" value={money(stats.data?.totalCompras)} /><Kpi label="Tickets" value={String(stats.data?.comprasCount ?? 0)} /><Kpi label="Ticket promedio" value={money(Number(stats.data?.totalCompras ?? 0) / Math.max(1, stats.data?.comprasCount ?? 0))} /><Kpi label="Metros" value={stats.data?.metros ?? "0"} /><Kpi label="Kilos" value={stats.data?.kilos ?? "0"} /><Kpi label="Costo identificable" value={money(stats.data?.costo)} /><Kpi label="Margen identificable" value={money(stats.data?.margen)} /></div>{(stats.data?.lineasSinCosto ?? 0) > 0 && <p className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Margen parcial: {stats.data?.lineasSinCosto} línea(s) no tienen costo congelado.</p>}<Card className="mt-4"><CardHeader><CardTitle>Compras por mes</CardTitle></CardHeader><CardContent>{chartData.length ? <div className="h-72" data-testid="chart-client-purchases"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip formatter={(value) => money(Number(value))} /><Bar dataKey="total" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div> : <p className="py-12 text-center text-muted-foreground">Sin datos para graficar en este periodo.</p>}</CardContent></Card></QueryState>}{canPrices && <QueryState query={prices}><Card className="mt-4"><CardHeader><CardTitle>Precios negociados recientes</CardTitle></CardHeader><CardContent><ResponsiveTable headers={["Fecha", "SKU", "Precio", "Promedio últimas 3"]} rows={(prices.data?.precios ?? []).map((item, index) => [date(item.fecha), item.sku, money(item.precioUnitario), money(item.promedio3), String(index)])} empty="No hay precios registrados." /></CardContent></Card></QueryState>}{canFinances && payments.data?.pagos?.length ? <p className="text-sm text-muted-foreground">{payments.data.pagos.length} pago(s) registrados en el historial.</p> : null}</TabsContent>}
...
function ClientAnalyticsBlocks({ query }: { query: { data?: Awaited<ReturnType<typeof getClientAnalytics>>; isLoading: boolean; isError: boolean; error: unknown } }) {
  return <div className="mt-4 space-y-4"><QueryState query={query}><div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle>Actividad del periodo</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-4"><Datum label="Última compra" value={query.data?.actividad.ultimaCompra ? date(query.data.actividad.ultimaCompra) : "Sin compras"} /><Datum label="Tickets" value={String(query.data?.actividad.tickets ?? 0)} /><Datum label="Ticket promedio" value={money(query.data?.actividad.ticketPromedio)} /><Datum label="Ticket máximo" value={money(query.data?.actividad.ticketMaximo)} /></CardContent></Card><Card><CardHeader><CardTitle>Mezcla de pagos</CardTitle></CardHeader><CardContent><ResponsiveTable headers={["Forma", "Importe", "Movimientos"]} rows={(query.data?.mezclaPagos ?? []).map((item, index) => [item.forma ?? "Sin forma", money(item.importe), String(item.movimientos), String(index)])} empty="Sin pagos en el periodo." /></CardContent></Card></div><Card><CardHeader><CardTitle>Tendencia mensual</CardTitle></CardHeader><CardContent><ResponsiveTable headers={["Mes", "Tickets", "Ventas"]} rows={(query.data?.tendencia ?? []).map((item, index) => [item.mes, String(item.tickets), money(item.ventas), String(index)])} empty="Sin tendencia disponible." /></CardContent></Card><Card><CardHeader><CardTitle>Productos por unidad, tela y color</CardTitle></CardHeader><CardContent><ResponsiveTable headers={["SKU", "Tela", "Color", "Unidad", "Cantidad", "Ventas", "Margen"]} rows={(query.data?.productos ?? []).map((item, index) => [item.sku, item.tela, item.color, item.unidad, item.cantidad, money(item.ventas), money(item.margen), String(index)])} empty="Sin productos en el periodo." /></CardContent></Card></QueryState></div>;
}
...
function Aging({ label, value }: { label: string; value?: string }) { return <div className="rounded-lg border p-4"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 font-semibold">{money(value)}</p></div>; }
