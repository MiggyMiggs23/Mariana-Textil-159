...
const table = (id: string, title: string, fields: Array<[string, string, string, boolean?]>, rows: Row[]) =>
  ({ id, title, columns: columns(fields), rows, totals: {} });
...
async function purchases(ctx: DomainReportContext): Promise<CommercialReport> {
  ...
  return {
    ...
    tables: [
      table("compras-por-rollo", "Costo unitario por rollo y compra", [["folio", "Folio", "text"], ["fecha", "Fecha", "text"], ["proveedor", "Proveedor", "text"], ["sku", "SKU", "text"], ["tela", "Tela", "text"], ["color", "Color", "text"], ["unidad", "Unidad", "text"], ["serie", "Rollo", "text"], ["cantidad", "Cantidad", "quantity"], ["costoUnitario", "Costo unitario", "money", true], ["costoTotal", "Costo total", "money", true]], rolls),
      table("proveedores", "Compras por proveedor", [["proveedor", "Proveedor", "text"], ["unidad", "Unidad", "text"], ["rollos", "Rollos", "count"], ["costo", "Costo", "money", true], ["concentracion", "Concentración por unidad", "percentage", true]], supplierRows),
      table("productos", "Compras por producto", [["sku", "SKU", "text"], ["tela", "Tela", "text"], ["color", "Color", "text"], ["unidad", "Unidad", "text"], ["cantidad", "Cantidad", "quantity"], ["rollos", "Rollos", "count"], ["costo", "Costo", "money", true]], byProduct.rows.map((r): Row => ({ sku:r.sku,tela:r.tela,color:r.color,unidad:r.unidad,cantidad:number(r.cantidad),rollos:number(r.rollos),costo:number(r.costo) }))),
      table("telas", "Compras por tela", [["tela", "Tela", "text"], ["cantidad", "Cantidad", "quantity"], ["rollos", "Rollos", "count"], ["costo", "Costo", "money", true]], byFabric.rows.map((r): Row => ({ tela:r.tela,cantidad:number(r.cantidad),rollos:number(r.rollos),costo:number(r.costo) }))),
      table("colores", "Compras por color", [["color", "Color", "text"], ["cantidad", "Cantidad", "quantity"], ["rollos", "Rollos", "count"], ["costo", "Costo", "money", true]], byColor.rows.map((r): Row => ({ color:r.color,cantidad:number(r.cantidad),rollos:number(r.rollos),costo:number(r.costo) }))),
...
async function clients(ctx: DomainReportContext): Promise<CommercialReport> {
  ...
  return { kpis: [{ id:"ventas-clientes",label:"Ventas",value:totalSales,kind:"money",economic:true },{ id:"utilidad-clientes",label:"Utilidad exacta",value:summary.rows.reduce((s,r)=>s+number(r.utilidad),0),kind:"money",economic:true},{id:"ticket-promedio",label:"Ticket promedio",value:tickets ? totalSales/tickets : 0,kind:"money",economic:true},{id:"clientes-nuevos",label:"Clientes nuevos",value:newClients,kind:"count"}],
    ...
    tables: [table("clientes","Clientes", [["cliente","Cliente","text"],["tickets","Tickets","count"],["ventas","Ventas","money",true],["utilidad","Utilidad exacta","money",true],["ticketPromedio","Ticket promedio","money",true],["frecuencia","Frecuencia","count"],["ultimaCompra","Última compra","text"],["riesgo","Riesgo","text"]],clientTable), table("publico-registrado","Público vs registrado",[["tipo","Tipo","text"],["tickets","Tickets","count"],["ventas","Ventas","money",true]],publicRows.rows.map((r):Row=>({tipo:r.es_sistema?"Público":"Registrado",tickets:number(r.tickets),ventas:number(r.ventas)}))),table("formas-pago","Métodos de pago",[["formaPago","Forma de pago","text"],["operaciones","Operaciones","count"],["importe","Importe","money",true]],payments.rows.map((r):Row=>({formaPago:r.forma_pago,operaciones:number(r.operaciones),importe:number(r.importe)}))),table("cuentas-por-cobrar-fifo","Cuentas por cobrar FIFO",[["clienteId","Cliente","count"],["fechaVencimiento","Vencimiento","text"],["saldo","Saldo","money",true],["cubeta","Antigüedad","text"],["vencida","Vencida","boolean"],["notas","Notas","text"]],aged), table("castigos-y-reversos","Castigos y reversos de crédito",[["clienteId","Cliente","count"],["tipo","Tipo","text"],["importe","Importe","money",true],["notas","Notas","text"],["fecha","Fecha","text"]],credit.rows.filter((r) => r.es_incobrable || r.tipo === "REVERSO").map((r): Row => ({clienteId:number(r.cliente_id),tipo:r.es_incobrable ? "INCOBRABLE" : r.tipo,importe:number(r.importe),notas:r.notas ?? null,fecha:new Date(r.created_at).toISOString()})))], warnings:["Las cuentas por cobrar incluyen únicamente movimientos de crédito vinculados a tickets dentro del alcance; los abonos sin ticket no se pueden atribuir a una ubicación o filtro de producto."] };
}
