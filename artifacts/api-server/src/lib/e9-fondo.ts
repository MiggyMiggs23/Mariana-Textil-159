...
import { E9_FONDO_INGRESS_ENABLED } from "./e9-feature";

export async function assertE9NoIndependentInverse(tx: FondoExecutor, id: string, enabled = E9_FONDO_INGRESS_ENABLED) {
  if (!enabled) return;
...
export async function readE9FundOrigin(tx: FondoExecutor, id: string, enabled = E9_FONDO_INGRESS_ENABLED) {
