...
router.get("/proveedores/resumen", async (_req, res, next): Promise<void> => {
  try {
...
// ── GET /proveedores (list with metrics) ──────────────────────────────────

router.get("/proveedores", async (_req, res, next): Promise<void> => {
  try {
    const resumen = await resumenProveedores();
    // Get full proveedor details for each
    const rows = await db
      .select()
      .from(proveedoresTable)
      .orderBy(proveedoresTable.nombre);

    // Merge metrics into proveedor data
    const metricsMap = new Map(
      resumen.items.map((i) => [i.proveedorId, i]),
    );
...
router.post("/proveedores", async (req, res, next): Promise<void> => {
  try {
...
router.get("/proveedores/:id", async (req, res, next): Promise<void> => {
  try {
...
router.patch("/proveedores/:id", async (req, res, next): Promise<void> => {
  try {
...
// ── POST /proveedores/:id/pagos (ADMIN) ───────────────────────────────────
...
router.post(
  ...
  async (req, res, next): Promise<void> => {
    try {
      ...
      // Validate entradaId belongs to proveedor if provided
      if (entradaId != null) {
...
// ── POST /proveedores/:id/ajustes (ADMIN) ─────────────────────────────────
