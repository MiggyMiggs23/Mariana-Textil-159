...
router.get("/admin/cuentas-destino", async (req, res, next): Promise<void> => {
  try {
    ...
    if (!filters) return;
    // Kept direct until the contract-first OpenAPI update is generated by the
    // owning agent; parsing with the previous generated schema would strip the
    // newly added reconciliation fields.
    res.json(await getDestinationAccounts(filters));
  } catch (error) {
    if (!badInput(error, res)) next(error);
  }
});
