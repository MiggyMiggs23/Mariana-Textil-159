...
// No environment dump, preflight output, command arguments or exception text.
// Exactly one O_APPEND write; never truncate, rotate or retry a partial write.
const [pid, stage, status, preflightExit, exitCode] = process.argv.slice(2);
let sha256 = null;
let hashError = null;
try {
  sha256 = createHash("sha256")
    .update(readFileSync("artifacts/api-server/dist/index.mjs")).digest("hex");
} catch (error) {
...
const entry = {
  ...
  bundle_hash_error: hashError,
  mode: {
    NODE_ENV: process.env.NODE_ENV ?? null,
    API_INSPECTION_BOOT: process.env.API_INSPECTION_BOOT ?? null,
  },
  preflight: { result: status, exit_code: preflightExit === "" ? null : Number(preflightExit) },
  stage,
  startup_exit_before_exec: stage === "exec_attempt" ? null : Number(exitCode),
};
let fd;
try {
