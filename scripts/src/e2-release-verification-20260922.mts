/** E2 CLOSED read-only comparison helpers and Drive-redownload restore verifier. */
...
const EXECUTED_BACKUP_DIRECTORY =
  `${BACKUPS}/e2-release-backup-20260922-20260922090513-1888`;
const EXECUTED_BACKUP_METADATA =
  `${REPORTS}/e2-backup-restore-20260922-metadata.json`;
const DRIVE_VERIFICATION = `${REPORTS}/drive-verification.json`;
const DRIVE_RESTORATION_REPORT = `${REPORTS}/drive-restoration.json`;
const SOURCE_SNAPSHOT = `${EXECUTED_BACKUP_DIRECTORY}/source-snapshot.json`;
const DOWNLOADED_DUMP = `${EXECUTED_BACKUP_DIRECTORY}/drive-redownload.dump`;
const ORIGINAL_DUMP =
  `${EXECUTED_BACKUP_DIRECTORY}/e2-release-backup-20260922-20260922090513-1888.dump`;
const EXECUTED_BACKUP_OPERATOR = `${ROOT}/scripts/src/e2-release-backup-20260922.mts`;
const APPROVAL =
...
export interface SequenceState extends Row {
  schema: string;
...
let backupDirectory = EXECUTED_BACKUP_DIRECTORY;
...
function progress(message: string): void {
  stage = message;
  console.error(`[e2-release-verification-20260922] ${message}`);
}
...
export async function sequenceState(client: Client): Promise<SequenceState[]> {
...
export async function readCatalogue(client: Client): Promise<Catalogue> {
...
export async function collectReadonlySnapshot(client: Client): Promise<E1Baseline> {
  const mode = await one<{ transaction_read_only: string }>(
    client,
    "SHOW transaction_read_only",
    [],
    "Verifying read-only comparison transaction",
  );
  if (mode.transaction_read_only !== "on") {
    throw new Error("Readonly snapshot verification requires transaction_read_only=on.");
  }
  return {
    catalogue: await readCatalogue(client),
    sequenceState: await sequenceState(client),
...
export async function restoreAndVerify(
  ...
  const cluster = `${backupDirectory}/drive-redownload-restore`;
  const socket = `/tmp/e2-drive-redownload-restore-${stampValue}-${process.pid}`;
  const admin = `e2_drive_restore_admin_${stampValue}`;
  const restored = `e2_drive_restore_disposable_${stampValue}`;
...
async function disabledBackupMain(): Promise<void> {
...
async function readJsonObject(path: string, label: string): Promise<Record<string, unknown>> {
  return object(JSON.parse(await fs.readFile(path, "utf8")) as unknown, label);
}

async function restoreDownloadedMain(): Promise<void> {
  if (process.argv.slice(2).length !== 1 || process.argv[2] !== "--restore-downloaded") {
    throw new Error("Only --restore-downloaded is accepted.");
  }
  let resultForReport: Record<string, unknown> = {
    status: "FAIL",
    verifiedAtUtc: new Date().toISOString(),
    backupDirectory: EXECUTED_BACKUP_DIRECTORY,
    downloadedDump: DOWNLOADED_DUMP,
    retainedCluster: `${EXECUTED_BACKUP_DIRECTORY}/drive-redownload-restore`,
    sourceDatabaseConnected: false,
    originalBackupFileModified: false,
  };
  try {
    if (backupDirectory !== EXECUTED_BACKUP_DIRECTORY) {
      throw new Error("Verification backup directory is not the pinned executed backup directory.");
    }
    for (const binary of ["pg_restore", "initdb", "pg_ctl", "psql"]) {
      if (!existsSync(`${PG_BIN}/${binary}`)) {
        throw new Error(`Required retained PostgreSQL 16 binary is missing: ${binary}.`);
      }
    }
    const [drive, backupMetadata, snapshotValue] = await Promise.all([
      readJsonObject(DRIVE_VERIFICATION, "Drive verification"),
      readJsonObject(EXECUTED_BACKUP_METADATA, "Executed backup metadata"),
      readJsonObject(SOURCE_SNAPSHOT, "Original source snapshot"),
    ]);
    const backupArchive = object(backupMetadata.archive, "Executed backup archive");
    const backupComparison = object(backupMetadata.comparison, "Executed backup comparison");
    const sourceProvenance = object(
      backupMetadata.sourceProvenance,
      "Executed backup source provenance",
    );
    const driveFile = object(drive.file, "Drive file");
    const snapshotArchive = object(snapshotValue.archive, "Source snapshot archive");
    const expectedSha256 = String(backupArchive.sha256 ?? "");
    const expectedSize = Number(backupArchive.sizeBytes);
    if (
      backupMetadata.status !== "PASS" ||
      backupComparison.status !== "PASS" ||
      backupMetadata.backupDirectory !== EXECUTED_BACKUP_DIRECTORY ||
      backupArchive.file !== ORIGINAL_DUMP
    ) {
      throw new Error("Original backup metadata is not PASS for the pinned backup directory.");
    }
    if (
      drive.status !== "PASS" ||
      drive.hashesMatch !== true ||
      drive.privatePermissionsVerified !== true ||
      drive.restoreOfDownloadedFile !== "PENDING" ||
      drive.downloadedPath !== DOWNLOADED_DUMP
    ) {
      throw new Error("Drive verification is not a private PASS with the pinned pending download.");
    }
    const permissions = object(drive.permissions, "Drive permissions");
    for (const id of [object(drive.folder, "Drive folder").id, driveFile.id]) {
      const entries = permissions[String(id)];
      if (
        !Array.isArray(entries) ||
        entries.length === 0 ||
        entries.some((entry) => {
          const permission = object(entry, "Drive permission");
          return permission.type !== "user" || permission.role !== "owner";
        })
      ) {
        throw new Error("Drive folder/file permissions are not owner-only.");
      }
    }
    if (
      !/^[a-f0-9]{64}$/.test(expectedSha256) ||
      drive.sourceSha256 !== expectedSha256 ||
      drive.downloadedSha256 !== expectedSha256 ||
      driveFile.sha256Checksum !== expectedSha256 ||
      snapshotArchive.sha256 !== expectedSha256 ||
      Number(drive.sizeBytes) !== expectedSize ||
      Number(driveFile.size) !== expectedSize
    ) {
      throw new Error("Drive, backup metadata, and source snapshot SHA/size evidence disagree.");
    }
    const [downloaded, original, executedOperator] = await Promise.all([
      sha256Size(DOWNLOADED_DUMP),
      sha256Size(ORIGINAL_DUMP),
      sha256Size(EXECUTED_BACKUP_OPERATOR),
    ]);
    if (
      downloaded.sha256 !== expectedSha256 ||
      downloaded.sizeBytes !== expectedSize ||
      original.sha256 !== expectedSha256 ||
      original.sizeBytes !== expectedSize ||
      executedOperator.sha256 !== sourceProvenance.operatorSha256
    ) {
      throw new Error("Downloaded or original local archive bytes do not match pinned SHA/size.");
    }
    if (existsSync(`${EXECUTED_BACKUP_DIRECTORY}/drive-redownload-restore`)) {
      throw new Error("Fresh Drive-redownload restore cluster path already exists.");
    }
    const restored = await restoreAndVerify(
      snapshotValue as unknown as Snapshot,
      DOWNLOADED_DUMP,
      "20260922090513-1888-drive",
    );
    resultForReport = {
      status: restored.comparison.status,
      verifiedAtUtc: new Date().toISOString(),
      driveVerification: DRIVE_VERIFICATION,
      originalBackupMetadata: EXECUTED_BACKUP_METADATA,
      sourceSnapshot: SOURCE_SNAPSHOT,
      backupDirectory: EXECUTED_BACKUP_DIRECTORY,
      downloadedDump: {
        file: DOWNLOADED_DUMP,
        sha256: downloaded.sha256,
        sizeBytes: downloaded.sizeBytes,
      },
      originalArchiveSha256: original.sha256,
      executedBackupOperatorSha256: executedOperator.sha256,
      evidenceSha256: expectedSha256,
      hashesMatch: true,
      restore: restored.metadata,
      comparison: restored.comparison,
      retainedCluster: restored.metadata.clusterDirectory,
      retainedForB0Comparison: true,
      sourceDatabaseConnected: false,
      originalBackupFileModified: false,
    };
    await writePrivate(DRIVE_RESTORATION_REPORT, resultForReport);
    if (restored.comparison.status !== "PASS") {
      throw new Error("Drive-redownload restore comparison reported discrepancies.");
    }
    console.log(JSON.stringify({
      status: "PASS",
      report: DRIVE_RESTORATION_REPORT,
      retainedCluster: restored.metadata.clusterDirectory,
    }, null, 2));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Drive-redownload verification failed.";
    if (resultForReport.status !== "PASS") {
      resultForReport = { ...resultForReport, status: "FAIL", failure: message };
      await writePrivate(DRIVE_RESTORATION_REPORT, resultForReport).catch(() => undefined);
    }
...
if (mainModulePath === fileURLToPath(import.meta.url)) {
  await restoreDownloadedMain().catch((error: unknown) => {
    console.error(`E2 Drive restore verification failed: ${
